...
class TestCoupledTranslation(unittest.TestCase):
    ...
    def test_full_size_fused_pcg_and_independent_widths(self):
        ...
        n = 1002
        device = "cuda:0"
        for width in (18, 6, 18):
            groups = np.concatenate([np.arange(841) % (width // 6), np.full(161, -1)]).astype(np.int32)
            points = rng.normal(size=(n, 3)).astype(np.float32)
            basis = np.zeros((n * 3, width))
            for row in range(841):
                group = groups[row]
                basis[3 * row : 3 * row + 3, 6 * group : 6 * group + 3] = np.eye(3)
                for axis in range(3):
                    basis[3 * row : 3 * row + 3, 6 * group + 3 + axis] = np.cross(np.eye(3)[axis], points[row] / 0.05)
            q = wp.array(points, dtype=wp.vec3, device=device)
            centers = wp.zeros(width // 6, dtype=wp.vec3, device=device)
            g = wp.array(groups, dtype=int, device=device)
            matrix = wp.array(np.eye(width, dtype=np.float32), device=device)
            inverse = wp.array(np.tile(np.eye(3), (n, 1, 1)), dtype=wp.mat33, device=device)
            _, update = _build_kernels(width)
            for iteration in (-1, 0, 3):
                x, r, z, p = [rng.normal(size=(n, 3)).astype(np.float32) for _ in range(4)]
                product = p.copy()
                expected_x, expected_r = x.astype(float), r.astype(float)
                if iteration >= 0:
                    alpha = 1.0 / np.sum(p.astype(float) ** 2)
                    expected_x += alpha * p
                    expected_r -= alpha * product
                expected_z = expected_r + (basis @ (basis.T @ expected_r.reshape(-1))).reshape(n, 3)
                expected_rz = np.sum(expected_r * expected_z)
                expected_p = expected_z + (expected_rz * p if iteration >= 0 else 0)
                arrays = [wp.array(a, dtype=wp.vec3, device=device) for a in (x, r, z, p, product)]
                ...
                wp.launch(
                    ...
                    dim=256,
                    block_dim=256,
                    inputs=[n, iteration, q, centers, g, matrix, *arrays, inverse, status, metrics, rz],
                    device=device,
                )
                for actual, expected in zip(arrays[:4], (expected_x, expected_r, expected_z, expected_p), strict=True):
                    np.testing.assert_allclose(actual.numpy(), expected, rtol=3e-4, atol=0.05)
                np.testing.assert_allclose(rz.numpy()[0], expected_rz, rtol=1e-5)
    ...
    def test_actual_augmented_step_against_dense_system(self):
        ...
        coupled = correction._coupled_solver
        pipeline = MJVBDV2CollisionPipeline(model, enable_rigid_soft_full_surface_contact=True)
        state, output = model.state(), model.state()
        contacts = pipeline.contacts()
        pipeline.collide(state, contacts)
        control = model.control()
        with self.assertRaisesRegex(ValueError, "Contacts buffer"):
            solver.step(state, output, control, None, 1 / 960)
