...
Five rigid shapes start on the table. The right hand picks them up one at a
...
Run from the repository root::
    ...
    uv run --extra examples -m newton.examples vbd_dexforce_throw_rigid_into_bag
"""
...
SHAPE_NAMES = ("mesh", "cone", "sphere", "box", "cylinder")
SHAPE_SIZE = 0.02
SHAPE_MARGIN = 0.005
SHAPE_DENSITY = 2000.0
SHAPE_POSITIONS = (
    wp.vec3(0.34, -0.22, TABLE_TOP_Z + 0.020),
    wp.vec3(0.42, -0.22, TABLE_TOP_Z + 0.020),
    wp.vec3(0.50, -0.22, TABLE_TOP_Z + 0.020),
    wp.vec3(0.58, -0.22, TABLE_TOP_Z + 0.020),
    wp.vec3(0.64, -0.22, TABLE_TOP_Z + 0.010),
)
SHAPE_COLORS = (
    (0.83, 0.56, 0.24),
    (0.90, 0.32, 0.18),
...
RIGHT_BAG_HOVER = wp.transform(
    wp.vec3(float(BAG_POS[0]) + 0.012 + GRASP_TCP_X_OFFSET, float(BAG_POS[1]) - 0.012, TABLE_TOP_Z + 0.15),
    RIGHT_GRASP_ROT,
)
...
@wp.kernel
def _hold_waiting_bodies(
    bodies: wp.array[int],
    released: wp.array[int],
    fixed_q: wp.array[wp.transform],
    carried_body: int,
    ...
    i = wp.tid()
    body = bodies[i]
    if released[i] == 0 and body != carried_body:
        body_q[body] = fixed_q[i]
        body_qd[body] = wp.spatial_vector()


@wp.kernel
def _release_waiting_body(
    object_index: int,
    body: int,
    released: wp.array[int],
    body_qd: wp.array[wp.spatial_vector],
...
def _carry_body(
    ...
    tcp_tf = wp.transform(
        wp.transform_get_translation(hand_tf) + wp.quat_rotate(hand_rot, tcp_offset),
        hand_rot,
    )
    carried_tf = wp.transform_multiply(tcp_tf, body_in_tcp)
    previous_tf = body_q[carried_body]
    linear_velocity = (wp.transform_get_translation(carried_tf) - wp.transform_get_translation(previous_tf)) * inv_dt
    delta_rot = wp.normalize(
        wp.transform_get_rotation(carried_tf) * wp.quat_inverse(wp.transform_get_rotation(previous_tf))
    )
    axis, angle = wp.quat_to_axis_angle(delta_rot)
    body_q[carried_body] = carried_tf
    body_qd[carried_body] = wp.spatial_vector(linear_velocity, axis * angle * inv_dt)
...
def _save_body_state(
    ...
    if wp.tid() == 0:
        saved_q[0] = src_q[body]
        saved_qd[0] = src_qd[body]
...
def _restore_body_state(
    ...
    if wp.tid() == 0:
        dst_q[body] = saved_q[0]
        dst_qd[body] = saved_qd[0]
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.object_body_indices = wp.array(self.object_bodies, dtype=wp.int32, device=self.device)
        self.object_initial_q = wp.array(
            self.state_0.body_q.numpy()[self.object_bodies].copy(),
            dtype=wp.transform,
            device=self.device,
        )
        self.object_released = wp.zeros(len(self.object_bodies), dtype=wp.int32, device=self.device)
        ...
        self.body_in_tcp = self._body_in_tcp_transforms()
        self.carried_q = wp.empty(1, dtype=wp.transform, device=self.device)
        self.carried_qd = wp.empty(1, dtype=wp.spatial_vector, device=self.device)
    ...
    def _build_scene(self):
        ...
        for name, position, color in zip(SHAPE_NAMES, SHAPE_POSITIONS, SHAPE_COLORS, strict=True):
            ...
            if name == "mesh":
                builder.add_shape_mesh(body, mesh=bear_mesh, cfg=cfg, color=color, label=f"pick_{name}_shape")
            elif name == "cone":
    ...
    def _segments(self):
        ...
        bag_hover = world(RIGHT_BAG_HOVER)
        retreat = world(
            wp.transform(
                wp.vec3(
                    float(BAG_POS[0]) + 0.012 + GRASP_TCP_X_OFFSET,
                    float(BAG_POS[1]) - 0.012,
                    TABLE_TOP_Z + 0.30,
                ),
                RIGHT_GRASP_ROT,
            )
        ...
        for object_index in range(len(self.object_bodies)):
            ...
            segments.extend(
                (
                    (0.65, self.left_home, self.left_home, right_start, approach, 0.0, 0.0, object_index),
                    (0.45, self.left_home, self.left_home, approach, grasp, 0.0, 0.0, object_index),
                    (0.40, self.left_home, self.left_home, grasp, grasp, 0.0, 1.0, object_index),
                    (0.55, self.left_home, self.left_home, grasp, lift, 1.0, 1.0, object_index),
                    (0.80, self.left_home, self.left_home, lift, bag_hover, 1.0, 1.0, object_index),
                    (0.25, self.left_home, self.left_home, bag_hover, bag_hover, 1.0, 1.0, object_index),
                    (0.35, self.left_home, self.left_home, bag_hover, bag_hover, 1.0, 0.0, object_index),
                    (0.45, self.left_home, self.left_home, bag_hover, retreat, 0.0, 0.0, object_index),
                )
            )
            right_start = retreat
    ...
    def _body_in_tcp_transforms(self):
        transforms = []
        for object_index in range(len(self.object_bodies)):
            grasp_tcp = self._world_tf(self._object_grasp_tf(object_index))
            object_tf = wp.transform(self._world_vec(SHAPE_POSITIONS[object_index]), self.base_rot)
            transforms.append(wp.transform_multiply(wp.transform_inverse(grasp_tcp), object_tf))
    ...
    def _prepare_frame(self):
        ...
        if self.carried_object < 0 and script_object >= 0 and grip >= 0.99:
            self.carried_object = script_object
        elif self.carried_object >= 0 and grip <= 1.0e-4:
            released_object = self.carried_object
            released_body = self.object_bodies[released_object]
            self.carried_object = -1
            wp.launch(
                _release_waiting_body,
                1,
                [released_object, released_body, self.object_released, self.state_0.body_qd],
                device=self.device,
            )
        return self.carried_object

    def simulate(self):
        carried_object = self._prepare_frame()
        carried_body = self.object_bodies[carried_object] if carried_object >= 0 else -1
        bag_dz = 0.0
        for substep in range(self.sim_substeps):
            ...
            wp.launch(
                _hold_waiting_bodies,
                self.object_body_indices.shape[0],
                [
                    self.object_body_indices,
                    self.object_released,
                    self.object_initial_q,
                    carried_body,
                    self.state_0.body_q,
            ...
            if carried_object >= 0:
                wp.launch(
                    ...
                    [
                        ...
                        carried_body,
                        TCP_OFFSET,
                        self.body_in_tcp[carried_object],
                        1.0 / self.sim_dt,
                        self.state_0.body_q,
                        self.state_0.body_qd,
                    ],
    ...
    def test_final(self):
        ...
        released = self.object_released.numpy()
    ...
    def create_parser():
        ...
        parser.set_defaults(num_frames=840)
