...
SHAPE_SIZE = 0.02
SHAPE_MARGIN = 0.005
SHAPE_DENSITY = 18000.0
SHAPE_POSITIONS = (
    wp.vec3(0.34, -0.22, TABLE_TOP_Z + 0.050),
    wp.vec3(0.42, -0.22, TABLE_TOP_Z + 0.050),
    wp.vec3(0.50, -0.22, TABLE_TOP_Z + 0.050),
    wp.vec3(0.58, -0.22, TABLE_TOP_Z + 0.050),
    wp.vec3(0.64, -0.22, TABLE_TOP_Z + 0.050),
)
SHAPE_COLORS = (
    (0.83, 0.56, 0.24),
    (0.90, 0.32, 0.18),
...
@wp.kernel
def _record_hand_object_contacts(
    rigid_contact_count: wp.array[int],
    rigid_contact_shape0: wp.array[int],
    rigid_contact_shape1: wp.array[int],
    object_shapes: wp.array[int],
    hand_shapes: wp.array[int],
    grasped: wp.array[int],
):
    contact = wp.tid()
    ...
    for object_index in range(object_shapes.shape[0]):
        object_shape = object_shapes[object_index]
        for hand_index in range(hand_shapes.shape[0]):
            hand_shape = hand_shapes[hand_index]
            if (shape0 == object_shape and shape1 == hand_shape) or (shape1 == object_shape and shape0 == hand_shape):
                wp.atomic_max(grasped, object_index, 1)


@wp.kernel
def _record_grasp_lifts(
    object_bodies: wp.array[int],
    initial_z: wp.array[float],
    grasped: wp.array[int],
    lifted: wp.array[int],
    ...
    object_index = wp.tid()
    if grasped[object_index] == 0:
        return
    position = wp.transform_get_translation(body_q[object_bodies[object_index]])
    if position[2] > initial_z[object_index] + lift_height:
        lifted[object_index] = 1
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.solver = newton.solvers.SolverMuJoCoVBD(
            ...
            vbd_bodies=self.object_bodies,
            vbd_particles=list(range(self.bag_particle_start, self.bag_particle_end)),
        ...
        self.object_shape_indices = wp.array(self.object_shapes, dtype=wp.int32, device=self.device)
        self.right_hand_shape_indices = wp.array(self.right_hand_shapes, dtype=wp.int32, device=self.device)
        self.object_initial_z = wp.array(
            self.state_0.body_q.numpy()[self.object_bodies, 2], dtype=float, device=self.device
        )
        self.object_grasped = wp.zeros(len(self.object_bodies), dtype=wp.int32, device=self.device)
        self.object_lifted = wp.zeros(len(self.object_bodies), dtype=wp.int32, device=self.device)
    ...
    def _prepare_frame(self):
        ...
        if self.carried_object < 0 and script_object >= 0 and grip >= 0.99:
            ...
            wp.launch(
                ...
                1,
                [released_object, self.object_released],
                device=self.device,
            )
    def simulate(self):
        self._prepare_frame()
        ...
        for substep in range(self.sim_substeps):
            ...
            wp.launch(
                ...
                [
                    ...
                    self.object_shape_indices,
                    self.right_hand_shape_indices,
                    self.object_grasped,
                ],
                device=self.device,
            )
            ...
            wp.launch(
                ...
                [
                    ...
                    self.solver.partition.vbd_entry.solver.body_particle_contact_overflow_max,
    ...
    def test_final(self):
        ...
        grasped = self.object_grasped.numpy()
        lifted = self.object_lifted.numpy()
        assert np.all(grasped == 1), f"Not all objects made physical hand contact: {grasped.tolist()}"
        assert np.all(lifted == 1), f"Not all grasped objects were physically lifted: {lifted.tolist()}"
