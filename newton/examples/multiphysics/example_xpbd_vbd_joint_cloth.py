...
###########################################################################
# Example: XPBD + VBD One-Way Coupling (Articulated Body + Cloth)
#
# Demonstrates one-way coupling between an articulated rigid body (XPBD)
# and cloth (VBD). A pendulum bar (revolute joint) swings back and forth
# while a cloth sheet drops onto it. The articulated body affects the
# cloth (collision response), but the cloth does NOT affect the
# articulated body.
#
# Capsule shapes are used for the bar because particle-shape collision
# cannot fully resolve sharp box corners — cloth particles tend to slip
# through corners due to discontinuous collision normals.
#
# Compare with:
#   - xpbd_vbd_rigid_cloth: Free rigid body + cloth
#   - xpbd_vbd_rigid_soft: Rigid body + soft body
#   - mujoco_vbd_joint_cloth_soft: MuJoCo articulated body + cloth & soft body
#
# Command: uv run -m newton.examples xpbd_vbd_joint_cloth
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        # Pendulum bar (XPBD) — pivot at top, bar swings down
        bar_length = 0.5
        bar_radius = 0.06
        pivot_z = 0.8
    ...
    def test_final(self):
        ...
        assert min_z > -0.5, f"Cloth exploded, min z={min_z:.3f}"
