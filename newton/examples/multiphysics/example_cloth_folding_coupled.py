...
###########################################################################
# Franka T-shirt folding with MuJoCo/VBD proxy coupling (V7 legacy-frame-corrected pinch grasp).
#
...
#   relaxation) so the arm tracks the IK targets undisturbed by cloth contact
#   forces while the garment follows the finger proxies through the VBD contact
#   constraint.  See --proxy-relaxation and --mass-scale.
...
# Shirt xy follows cloth_franka's authored transform. The z is computed in
# Example._add_shirt so the garment's lowest particle starts just above the
# table: the unisex_shirt asset is a ~28 cm tall 3-D garment, so cloth_franka's
# z=0.30 m origin buries its lowest particles ~10 cm inside the table. That
# penetration is tolerated by cm-scale VBD but, in this SI-scale coupled scene,
# produces a contact impulse that launches the cloth through the table.
SHIRT_POS = wp.vec3(0.0, 0.70, 0.30)
SHIRT_SETTLE_CLEARANCE = 0.015  # [m] gap above the table so the garment settles without penetration
GRIP_FORCE = 1000.0
...
class Example:
    ...
    def _build_scene(self) -> None:
        ...
        first_grasp_xy = np.array([0.31, -0.60], dtype=np.float32)
    ...
    def _add_shirt(self, builder: newton.ModelBuilder) -> None:
        ...
        # The 180°-about-z rotation leaves z unchanged, so the lowest world-z
        # particle is pos.z + min(vertex.z) * CM_TO_M.  Lift the origin so that
        # particle starts SHIRT_SETTLE_CLEARANCE above the table top instead of
        # ~10 cm inside it (see the SHIRT_POS comment for why this matters here
        # but not in cm-scale cloth_franka).
        min_z_local = float(np.min(vertices_np[:, 2])) * CM_TO_M
        table_top = float(TABLE_POS[2] + TABLE_HALF_EXTENTS[2])
        shirt_pos = wp.vec3(
            float(SHIRT_POS[0]),
            float(SHIRT_POS[1]),
            table_top + SHIRT_SETTLE_CLEARANCE - min_z_local,
        )
            ...
            coupling=SolverCoupledProxy.Config(
                proxies=[
                    SolverCoupledProxy.Proxy(
                        ...
                        # coupling force at its initial value (zero), so the
                        # MuJoCo arm never feels the cloth.  The arm therefore
                        # tracks the IK targets as well as a standalone MuJoCo
                        # solve (verified: <1 cm TCP error at the grasp pose),
                        # while a large mass_scale makes the VBD finger proxies
                        # near-kinematic so the cloth follows them through the
                        # contact constraint.  This mirrors cloth_franka's
                        # kinematic-robot coupling but reuses SolverCoupledProxy.
    ...
    def _build_keyframes(self) -> None:
        ...
        motion_poses_cm = np.array(
            [
                ...
                [2.0, 31.0, -60.0, 20.0, *qa, open_],
                [2.0, 31.0, -60.0, 20.0, *qa, close],
                [2.0, 26.0, -60.0, 26.0, *qa, close],
        ...
        if hold > 0.0:
            # Rows where closing has just completed in the original sequence.
            close_rows = {2, 9, 15, 20, 26}
            expanded = []
        ...
        if bool(self.args.first_grasp_only):
            ...
            motion_poses_cm = np.array(
                [
                    ...
                    [2.0, 31.0, -60.0, 20.0, *qa, open_],
                    [1.5, 31.0, -60.0, 20.0, *qa, close],
                    [max(hold, 1.0), 31.0, -60.0, 20.0, *qa, close],
                    [2.0, 31.0, -60.0, 32.0, *qa, close],
                    [1.5, 31.0, -60.0, 32.0, *qa, close],
                ],
                dtype=np.float32,
            )
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            type=float,
            default=100.0,
            help="Proxy effective mass/inertia scale.  Large values make the VBD finger proxies near-kinematic so the cloth follows them.",
        )
        parser.add_argument(
            "--coupling-mode",
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.0,
            help="Feedback relaxation factor.  0.0 = one-way coupling (no force fed back to MuJoCo); 1.0 = full two-way feedback.",
        )
        parser.add_argument(
