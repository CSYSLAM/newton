...
This file targets Newton v1.4.0.  ``SolverCoupledProxy`` is currently under the
...
class Example:
    def __init__(self, viewer, args):
        ...
        print(f"[INFO] Hand proxy bodies={len(self.hand_proxy_bodies)}")
    ...
    def _add_realistic_workbench(self, builder: newton.ModelBuilder) -> None:
        ...
        builder.add_ground_plane(cfg=collision_cfg)
    ...
    def _manipulation_targets(self, task_t: float) -> tuple[np.ndarray, np.ndarray, float]:
        left_pre = LEFT_OBJECT_CENTER + np.array([0.0, -0.055, 0.13], dtype=np.float32)
        right_pre = RIGHT_OBJECT_CENTER + np.array([0.0, -0.055, 0.13], dtype=np.float32)
        left_grasp = LEFT_OBJECT_CENTER + np.array([0.0, 0.0, 0.0], dtype=np.float32)
        right_grasp = RIGHT_OBJECT_CENTER + np.array([0.0, 0.0, 0.0], dtype=np.float32)
        left_lift = LEFT_OBJECT_CENTER + np.array([-0.055, -0.045, 0.22], dtype=np.float32)
        right_lift = RIGHT_OBJECT_CENTER + np.array([0.055, -0.045, 0.22], dtype=np.float32)

        if task_t < 1.4:
            alpha = task_t / 1.4
            return left_pre, right_pre, 0.0
        if task_t < 2.6:
            alpha = (task_t - 1.4) / 1.2
            return _interp(left_pre, left_grasp, alpha), _interp(right_pre, right_grasp, alpha), 0.0
        if task_t < 3.6:
            alpha = (task_t - 2.6) / 1.0
            return left_grasp, right_grasp, _smoothstep01(alpha)
        if task_t < 5.2:
            alpha = (task_t - 3.6) / 1.6
    ...
    def _update_task_controller(self) -> None:
        ...
        if self.phase == "approach":
            if root_y < ROBOT_STOP_Y:
                self.command = wp.vec3(float(self.args.walk_speed), 0.0, 0.0)
            else:
                self.phase = "settle"
        ...
        self._run_policy()
        if self.phase == "manipulate" and self.manip_start_time is not None:
    ...
    def create_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument("--walk-speed", type=float, default=0.32, help="G1 forward velocity command [m/s].")
        parser.add_argument("--settle-time", type=float, default=1.2, help="Balance settling time before reaching [s].")
        ...
        parser.add_argument("--mujoco-iterations", type=int, default=20)
        parser.add_argument("--mujoco-ls-iterations", type=int, default=5)
        parser.add_argument("--vbd-iterations", type=int, default=32)
        parser.add_argument("--proxy-iterations", type=int, default=2)
        parser.add_argument("--proxy-mass-scale", type=float, default=0.75)
        parser.add_argument("--proxy-relaxation", type=float, default=0.7)
        parser.add_argument(
