...
###########################################################################
# Example MuJoCo-VBD Cloth Franka Coupled Solver Interactive
#
# Same setup as ``example_mujoco_vbd_cloth_franka_coupled_solver`` but with
...
#
# Command: python -m newton.examples mujoco_vbd_cloth_franka_coupled_solver_interactive
#          python -m newton.examples mujoco_vbd_cloth_franka_coupled_solver_interactive --disable-franka
#
###########################################################################
...
DEBUG_CUBE_SIZE = 0.01  # meters, half-extents
DEBUG_CUBE_COLOR = wp.vec3(1.0, 0.2, 0.2)  # red
SLEEVE_GRASP_PROBE = np.array([0.304, -0.558, 0.236], dtype=np.float32)
target1 = np.array([0.34, -0.60, 0.035], dtype=np.float32)
...
class Example:
    ...
    def _add_franka(self, builder: newton.ModelBuilder) -> None:
        ...
        self.robot_key_poses = np.array(
            [
                [0.9, 0.38, -0.56, 0.25, *grasp_quat, gripper_open],
                [0.7, 0.35, -0.58, 0.08, *grasp_quat, gripper_open],
                [0.9, 0.34, -0.60, 0.00, *grasp_quat, gripper_closed],
                [0.4, 0.34, -0.60, 0.02, *grasp_quat, gripper_closed],
                [0.8, 0.292, -0.548, 0.10, *grasp_quat, gripper_closed],
                [1.1, 0.250, -0.520, 0.16, *grasp_quat, gripper_closed],
                [1.2, 0.200, -0.500, 0.22, *grasp_quat, gripper_closed],
            ],
            dtype=np.float32,
        )
    ...
    def _select_grasp_particles(self) -> None:
        ...
        distance = np.linalg.norm(particle_q - SLEEVE_GRASP_PROBE, axis=1)
    ...
    def render(self) -> None:
        ...
        target = self._interpolated_target()
        cube_pos = wp.vec3(float(target1[0]), float(target1[1]), float(target1[2]))
        ...
        self.viewer.log_shapes(
            ...
            wp.array([wp.transform(cube_pos, cube_rot)], dtype=wp.transform),
