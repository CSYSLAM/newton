...
def set_indexed_joint_q_kernel(
    ...
    knife_grasp_values: wp.array[wp.float32],
    pan_grasp_values: wp.array[wp.float32],
    alpha: float,
    grip_mode: float,
    joint_q_out: wp.array[wp.float32],
):
    """Blend hand joints between open, knife grasp, and pan-handle grasp."""
    i = wp.tid()
    grasp = knife_grasp_values[i] * (1.0 - grip_mode) + pan_grasp_values[i] * grip_mode
    joint_q_out[q_indices[i]] = open_values[i] * (1.0 - alpha) + grasp * alpha
...
@wp.kernel
def carry_particles_with_transform_delta_kernel(
    particle_q: wp.array[wp.vec3],
    particle_qd: wp.array[wp.vec3],
    prev_tf: wp.transform,
    next_tf: wp.transform,
    inv_dt: float,
):
    """Rigidly carry meat particles by the pan transform delta."""
    i = wp.tid()
    p0 = particle_q[i]
    local = wp.transform_point(wp.transform_inverse(prev_tf), p0)
    p1 = wp.transform_point(next_tf, local)
    particle_q[i] = p1
    particle_qd[i] = (p1 - p0) * inv_dt
...
PAN_RIM_COLOR = (1.0, 0.62, 0.12)
PAN_GRIP_TCP_LOCAL = wp.vec3(float(PAN_HANDLE_LOCAL_POS[0]), float(PAN_HANDLE_LOCAL_POS[1]), PAN_DISK_HALF_HEIGHT + 0.050)
TUNED_PAN_GRASP_TCP_TF = wp.transform(
    wp.vec3(5.437493, 0.305373, 0.921815),
    wp.quat(0.512743, 0.312141, 0.393223, 0.696447),
)
PAN_LIFT_HEIGHT = 0.24
PAN_ROOT_TURN_YAW = math.radians(90.0)
REAR_TABLE_PAN_CENTER_WORLD = wp.vec3(4.24, 1.05, WAIC_WORKTOP_Z + PAN_DISK_HALF_HEIGHT)
REAR_TABLE_PAN_YAW = math.radians(90.0)
...
PAN_HANDLE_WRIST_QUAT = RIGHT_WRIST_QUAT
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.show_static_visual_pan = bool(args.show_static_visual_pan)
        ...
        self.rear_table_pan_center = wp.vec3(
            float(args.rear_table_pan_center_x),
            float(args.rear_table_pan_center_y),
            float(args.rear_table_pan_center_z),
        )
        self.pan_top_z = float(self.pan_center[2]) + PAN_DISK_HALF_HEIGHT
        self.pan_initial_tf = wp.transform(self.pan_center, self.scene_rotation)
        self.pan_turn_tf = self._pan_orbit_transform(
            angle=PAN_ROOT_TURN_YAW,
            z=float(self.pan_center[2]) + PAN_LIFT_HEIGHT,
            yaw=PAN_ROOT_TURN_YAW,
        )
        self.pan_lift_tf = self._pan_world_transform(
            wp.vec3(float(self.pan_center[0]), float(self.pan_center[1]), float(self.pan_center[2]) + PAN_LIFT_HEIGHT),
            0.0,
        )
        self.pan_carry_mid_tf = self._pan_orbit_transform(angle=0.5 * PAN_ROOT_TURN_YAW, z=float(self.pan_center[2]) + PAN_LIFT_HEIGHT, yaw=0.5 * PAN_ROOT_TURN_YAW)
        self.pan_place_above_tf = self._pan_world_transform(
            wp.transform_get_translation(self.pan_turn_tf),
            PAN_ROOT_TURN_YAW,
        )
        self.pan_place_tf = self.pan_turn_tf
        self.pan_handle_grasp_tf = self._normalize_transform(TUNED_PAN_GRASP_TCP_TF)
        self.pan_handle_approach_tf = self._translate_transform(self.pan_handle_grasp_tf, wp.vec3(0.0, 0.0, 0.14))
        self.pan_lift_hand_tf = self._translate_transform(self.pan_handle_grasp_tf, wp.vec3(0.0, 0.0, PAN_LIFT_HEIGHT))
        self.pan_carry_mid_hand_tf = self._orbit_transform_from_start(self.pan_lift_hand_tf, 0.5 * PAN_ROOT_TURN_YAW, 0.5 * PAN_ROOT_TURN_YAW)
        self.pan_place_above_hand_tf = self._orbit_transform_from_start(self.pan_lift_hand_tf, PAN_ROOT_TURN_YAW, PAN_ROOT_TURN_YAW)
        self.pan_place_hand_tf = self.pan_place_above_hand_tf
        self.pan_release_retreat_tf = self._translate_transform(self.pan_place_hand_tf, wp.vec3(0.0, 0.0, 0.16))
        ...
        self._pan_prev_pos = wp.transform_get_translation(self.pan_initial_tf)
        self.pan_tf = self.pan_initial_tf
        self.pan_frame_start_tf = self.pan_initial_tf
        self.pan_frame_end_tf = self.pan_initial_tf
        ...
        ) = self._build_right_hand_targets()
        self.setup_ik()
    ...
    def _add_simplified_pan(self, builder: newton.ModelBuilder) -> None:
        ...
        for i in range(8):
            theta = 2.0 * math.pi * float(i) / 8.0
            c = float(math.cos(theta))
            s = float(math.sin(theta))
            builder.add_shape_box(
                body=self.pan_body,
                xform=wp.transform(
                    wp.vec3(c * (PAN_RADIUS + 0.003), s * (PAN_RADIUS + 0.003), PAN_DISK_HALF_HEIGHT + 0.022),
                    wp.quat(0.0, 0.0, float(math.sin(0.5 * theta)), float(math.cos(0.5 * theta))),
                ),
                hx=0.006,
                hy=PAN_RADIUS * 0.23,
                hz=0.026,
    ...
    def _build_motion_segments(self):
        # (duration, right_start, right_end, hand_start, hand_end,
        #  knife_start, knife_end, pan_start, pan_end,
        #  grip_start, grip_end, carry_start, carry_end)
        # hand alpha: 0 = hand open, 1 = fully pinching.
        # knife alpha: 0 = knife at rest, 1 = knife follows the right TCP.
        # grip mode: 0 = knife grasp, 1 = pan-handle grasp.
        # carry alpha: when 1, meat particles are rigidly carried with the pan.
        # pan transform drives the simplified kinematic frying-pan collider.
        ...
        #   8. carry the pan and meat to the rear table, then release
        knife_rest_tf = self.knife_grasp_tf
        knife_lift_tf = self.knife_lift_tf
        pan_initial_tf = self.pan_initial_tf
        knife_grip = 0.0
        pan_grip = 1.0
        free_meat = 0.0
        carry_meat = 1.0
        ...
        def seg(
            ...
            knife_start,
            knife_end,
            pan_start=pan_initial_tf,
            pan_end=pan_initial_tf,
            grip_start=knife_grip,
            grip_end=knife_grip,
            carry_start=free_meat,
            carry_end=free_meat,
        ):
            return (
                duration,
        ...
        return (
            seg(1.2, knife_rest_tf, knife_rest_tf, 0.0, 1.0, 0.0, 1.0),
            seg(0.8, knife_rest_tf, knife_lift_tf, 1.0, 1.0, 1.0, 1.0),
            seg(1.2, knife_lift_tf, self.right_above_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.8, self.right_above_tf, self.right_descend_tf, 1.0, 1.0, 1.0, 1.0),
            seg(3.2, self.right_descend_tf, self.right_cut_end_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.8, self.right_cut_end_tf, self.right_above_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.8, self.right_above_tf, knife_lift_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.8, knife_lift_tf, knife_rest_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.5, knife_rest_tf, knife_rest_tf, 1.0, 1.0, 1.0, 1.0),
            seg(0.6, knife_rest_tf, knife_rest_tf, 1.0, 0.0, 1.0, 0.0),
            seg(0.4, knife_rest_tf, knife_rest_tf, 0.0, 0.0, 0.0, 0.0),
            seg(0.8, knife_rest_tf, self.pan_handle_approach_tf, 0.0, 0.0, 0.0, 0.0, grip_start=pan_grip, grip_end=pan_grip),
            seg(0.8, self.pan_handle_approach_tf, self.pan_handle_grasp_tf, 0.0, 0.0, 0.0, 0.0, grip_start=pan_grip, grip_end=pan_grip),
            seg(0.8, self.pan_handle_grasp_tf, self.pan_handle_grasp_tf, 0.0, 1.0, 0.0, 0.0, grip_start=pan_grip, grip_end=pan_grip),
            seg(0.5, self.pan_handle_grasp_tf, self.pan_handle_grasp_tf, 1.0, 1.0, 0.0, 0.0, grip_start=pan_grip, grip_end=pan_grip),
            seg(
                1.7,
                self.pan_handle_grasp_tf,
                self.pan_lift_hand_tf,
                1.0,
                1.0,
                0.0,
                0.0,
            ...
            seg(
                2.6,
                self.pan_lift_hand_tf,
                self.pan_carry_mid_hand_tf,
                1.0,
                1.0,
                0.0,
                0.0,
                self.pan_lift_tf,
                self.pan_carry_mid_tf,
                pan_grip,
                pan_grip,
                carry_meat,
                carry_meat,
            ...
            seg(
                2.6,
                self.pan_carry_mid_hand_tf,
                self.pan_place_above_hand_tf,
                1.0,
                1.0,
                0.0,
                0.0,
                self.pan_carry_mid_tf,
                self.pan_place_above_tf,
                pan_grip,
                pan_grip,
                carry_meat,
                carry_meat,
            ...
            seg(0.7, self.pan_place_hand_tf, self.pan_place_hand_tf, 1.0, 1.0, 0.0, 0.0, self.pan_place_tf, self.pan_place_tf, pan_grip, pan_grip, carry_meat, carry_meat),
            seg(0.7, self.pan_place_hand_tf, self.pan_place_hand_tf, 1.0, 0.0, 0.0, 0.0, self.pan_place_tf, self.pan_place_tf, pan_grip, pan_grip, carry_meat, carry_meat),
            seg(1.0, self.pan_place_hand_tf, self.pan_release_retreat_tf, 0.0, 0.0, 0.0, 0.0, self.pan_place_tf, self.pan_place_tf, pan_grip, pan_grip, carry_meat, carry_meat),
        )

    def _sample_script(self, t: float) -> tuple[wp.transform, float, float, wp.transform, float, float]:
        """Return right TCP, hand alpha, knife alpha, pan transform, grip mode, carry alpha."""
        remaining = t
        for (
            duration,
            start,
            ...
            if remaining <= duration:
                ...
                pan_tf = self._interpolate_transform(pan_start, pan_end, alpha)
                grip_mode = grip_start * (1.0 - alpha) + grip_end * alpha
                carry_alpha = carry_start * (1.0 - alpha) + carry_end * alpha
                return right_tf, hand_alpha, knife_alpha, pan_tf, grip_mode, carry_alpha
            remaining -= duration
        _, _, right_end, _, hand_end, _, knife_end, _, pan_end, _, grip_end, _, carry_end = self.motion_segments[-1]
        return right_end, hand_end, knife_end, pan_end, grip_end, carry_end

    # ------------------------------------------------------------------ step
    def _prepare_frame_targets(self) -> None:
        wp.copy(self.frame_joint_q_start, self.state_0.joint_q)
        self.pan_frame_start_tf = self.pan_tf
        (
            self.right_tf,
            grasp_alpha,
            self.knife_alpha,
            self.pan_frame_end_tf,
            self.grip_mode,
            self.meat_carry_alpha,
        ) = self._sample_script(self.sim_time + self.frame_dt)
        self.pan_tf = self.pan_frame_end_tf
    ...
    def _update_pan_transform(self, pan_tf: wp.transform) -> None:
        """Drive the simplified frying pan as a kinematic collider."""
        prev_pos = self._pan_prev_pos
        pan_pos = wp.transform_get_translation(pan_tf)
        vel = (pan_pos - prev_pos) * (1.0 / self.sim_dt)
        self._pan_prev_pos = pan_pos
    ...
    def simulate(self) -> None:
        ...
        for substep in range(self.sim_substeps):
            ...
            pan_substep_start_tf = self._interpolate_transform(
                self.pan_frame_start_tf,
                self.pan_frame_end_tf,
                float(substep / self.sim_substeps),
            )
            pan_substep_tf = self._interpolate_transform(self.pan_frame_start_tf, self.pan_frame_end_tf, substep_alpha)
            # Once the hand is closing, drive the knife to follow the TCP.
            self._update_knife_transform(self.knife_alpha)
            self._update_pan_transform(pan_substep_tf)
            ...
            if self.meat_carry_alpha > 0.5:
                wp.launch(
                    carry_particles_with_transform_delta_kernel,
                    dim=self.model.particle_count,
                    inputs=[
                        self.state_0.particle_q,
                        self.state_0.particle_qd,
                        pan_substep_start_tf,
                        pan_substep_tf,
                        1.0 / self.sim_dt,
                    ],
                    device=self.model.device,
    ...
    def _pan_world_transform(self, center: wp.vec3, yaw: float) -> wp.transform:
        yaw_quat = wp.quat(0.0, 0.0, float(math.sin(0.5 * yaw)), float(math.cos(0.5 * yaw)))
        return wp.transform(center, self._quat_multiply(self.scene_rotation, yaw_quat))

    def _pan_orbit_transform(self, angle: float, z: float, yaw: float) -> wp.transform:
        pivot = np.array([float(self.waic_robot_base_pos[0]), float(self.waic_robot_base_pos[1])], dtype=np.float64)
        start = np.array([float(self.pan_center[0]), float(self.pan_center[1])], dtype=np.float64)
        rel = start - pivot
        c = math.cos(angle)
        s = math.sin(angle)
        rotated = np.array([rel[0] * c - rel[1] * s, rel[0] * s + rel[1] * c], dtype=np.float64)
        center = wp.vec3(float(pivot[0] + rotated[0]), float(pivot[1] + rotated[1]), z)
        return self._pan_world_transform(center, yaw)

    def _orbit_transform_from_start(self, tf: wp.transform, angle: float, yaw: float) -> wp.transform:
        pivot = np.array([float(self.waic_robot_base_pos[0]), float(self.waic_robot_base_pos[1])], dtype=np.float64)
        pos = wp.transform_get_translation(tf)
        start = np.array([float(pos[0]), float(pos[1])], dtype=np.float64)
        rel = start - pivot
        c = math.cos(angle)
        s = math.sin(angle)
        rotated = np.array([rel[0] * c - rel[1] * s, rel[0] * s + rel[1] * c], dtype=np.float64)
        new_pos = wp.vec3(float(pivot[0] + rotated[0]), float(pivot[1] + rotated[1]), float(pos[2]))
        yaw_quat = wp.quat(0.0, 0.0, float(math.sin(0.5 * yaw)), float(math.cos(0.5 * yaw)))
        new_rot = self._quat_multiply(yaw_quat, wp.transform_get_rotation(tf))
        return wp.transform(new_pos, new_rot)

    def _translate_transform(self, tf: wp.transform, delta: wp.vec3) -> wp.transform:
        pos = wp.transform_get_translation(tf)
        return wp.transform(
            wp.vec3(float(pos[0]) + float(delta[0]), float(pos[1]) + float(delta[1]), float(pos[2]) + float(delta[2])),
            wp.transform_get_rotation(tf),
        )

    def _normalize_transform(self, tf: wp.transform) -> wp.transform:
        return wp.transform(wp.transform_get_translation(tf), self._normalize_wp_quat(wp.transform_get_rotation(tf)))

    def _pan_grip_tcp_transform(self, pan_tf: wp.transform, lift_extra: float = 0.0) -> wp.transform:
        pan_pos = wp.transform_get_translation(pan_tf)
        pan_rot = wp.transform_get_rotation(pan_tf)
        local = wp.vec3(
            float(PAN_GRIP_TCP_LOCAL[0]),
            float(PAN_GRIP_TCP_LOCAL[1]),
            float(PAN_GRIP_TCP_LOCAL[2]) + lift_extra,
        )
        tcp_pos = pan_pos + wp.quat_rotate(pan_rot, local)
        tcp_rot = self._quat_multiply(pan_rot, PAN_HANDLE_WRIST_QUAT)
        return wp.transform(tcp_pos, tcp_rot)
    ...
    def _add_waic_visual_model(self, builder: newton.ModelBuilder) -> None:
        ...
        for prim in stage.Traverse():
            ...
            if not self.show_static_visual_pan and self._is_static_visual_pan_prim(prim_path):
                continue
    ...
    @staticmethod
    def _is_static_visual_pan_prim(prim_path: str) -> bool:
        return "frying_pan" in prim_path.lower()
    ...
    def _build_right_hand_targets(self) -> tuple[wp.array, wp.array, wp.array, wp.array]:
        """Build open, knife-grasp and pan-handle-grasp values.

        Knife grasp is a pinching pose. Pan grasp curls the long fingers more
        deeply around the handle and uses a stronger opposed thumb so the hand
        reads as wrapping a thicker handle instead of pinching a blade.
        """
        q_home = self.model.joint_q.numpy()
        q_start = self.model.joint_q_start.numpy()
        knife_fraction = {
        ...
        pan_values_tuned = {
            "RIGHT_HAND_THUMB2": 0.59690,
            "RIGHT_HAND_THUMB1": 0.68067,
            "RIGHT_HAND_INDEX": 0.96866,
            "RIGHT_INDEX_PIP": 1.38230,
            "RIGHT_HAND_MIDDLE": 1.02102,
            "RIGHT_MIDDLE_PIP": 1.42419,
            "RIGHT_HAND_RING": 0.99484,
            "RIGHT_RING_PIP": 1.38230,
            "RIGHT_HAND_PINKY": 0.91630,
            "RIGHT_PINKY_PIP": 1.25664,
        }
        q_indices = []
        open_values = []
        knife_values = []
        ...
        for joint_name in self.RIGHT_HAND_JOINTS:
            ...
            knife_values.append(upper * knife_fraction[joint_name])
            pan_values.append(pan_values_tuned[joint_name])
        return (
            wp.array(q_indices, dtype=wp.int32, device=self.model.device),
            wp.array(open_values, dtype=wp.float32, device=self.model.device),
            wp.array(knife_values, dtype=wp.float32, device=self.model.device),
            wp.array(pan_values, dtype=wp.float32, device=self.model.device),
        )
    ...
    def create_parser():
        ...
        parser.set_defaults(num_frames=1700)
        ...
        parser.add_argument("--rear-table-pan-center-x", type=float, default=float(REAR_TABLE_PAN_CENTER_WORLD[0]))
        parser.add_argument("--rear-table-pan-center-y", type=float, default=float(REAR_TABLE_PAN_CENTER_WORLD[1]))
        parser.add_argument("--rear-table-pan-center-z", type=float, default=float(REAR_TABLE_PAN_CENTER_WORLD[2]))
        parser.add_argument(
            "--waic-worktop-z",
            type=float,
        ...
        parser.add_argument(
            "--show-static-visual-pan",
            action="store_true",
            help="Keep the Blender-exported static pan mesh visible in addition to the moving simplified Newton pan.",
        )
        parser.add_argument(
            "--show-physics-table",
            action="store_true",
