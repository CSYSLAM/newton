...
# Meat block: a low, wide patty centered on the table, in front of the robot.
MEAT_LO = np.array([0.48, -0.14, TABLE_TOP_Z])
MEAT_HI = np.array([0.78, 0.14, TABLE_TOP_Z + 0.06])
...
# Right-arm key poses (TCP target transforms). The blade tip reaches these.
# Meat spans x in [0.48, 0.78], y in [-0.14, 0.14], z in [1.175, 1.235].
# Cut along +y, sweeping x across the meat at a fixed depth.
# Wrist orientation: blade edge along y, pointing down (-z).
BLADE_DOWN_QUAT = wp.quat_from_axis_angle(wp.vec3(0.0, 1.0, 0.0), np.pi)  # flip so blade points -x (forward)


def _tcp(x: float, y: float, z: float) -> wp.transform:
    return wp.transform(wp.vec3(x, y, z), BLADE_DOWN_QUAT)


# Approach above the meat, then cut across +y, lift, return.
RIGHT_HOME_TF = _tcp(0.63, -0.22, TABLE_TOP_Z + 0.30)
RIGHT_APPROACH_TF = _tcp(0.63, -0.20, TABLE_TOP_Z + 0.10)
# Cut entry: blade tip just above meat surface at -y edge
RIGHT_CUT_START_TF = _tcp(0.63, -0.18, TABLE_TOP_Z + 0.01)
# Cut exit: blade tip past +y edge of meat
RIGHT_CUT_END_TF = _tcp(0.63, 0.18, TABLE_TOP_Z + 0.01)
RIGHT_LIFT_TF = _tcp(0.63, 0.20, TABLE_TOP_Z + 0.18)
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.blade_shape = self._add_blade(builder)
    ...
    def _add_blade(self, builder: newton.ModelBuilder) -> int:
        rj7 = self._builder_body_index(builder, "right_j7")
        blade_cfg = newton.ModelBuilder.ShapeConfig()
        blade_cfg.ke = 1.0e6
        blade_cfg.kd = 1.0e-6
        blade_cfg.mu = 0.2
        blade_cfg.density = 0.0
        # Local transform: blade extends from the wrist along -x (TCP_OFFSET),
        # so its center sits at half the TCP offset.
    ...
    def _build_motion_segments(self):
        ...
        # Open while approaching, close to grip before the cut, stay closed
        # through the cut and lift, open again after returning home.
        return (
            (0.8, RIGHT_HOME_TF, RIGHT_APPROACH_TF, 0.0, 0.0),
            (0.6, RIGHT_APPROACH_TF, RIGHT_CUT_START_TF, 0.0, 1.0),
            (1.8, RIGHT_CUT_START_TF, RIGHT_CUT_END_TF, 1.0, 1.0),
            (0.5, RIGHT_CUT_END_TF, RIGHT_LIFT_TF, 1.0, 1.0),
            (0.8, RIGHT_LIFT_TF, RIGHT_HOME_TF, 1.0, 0.0),
        )
