...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.meat_lo = np.array([
            ...
            self.pan_top_z + 0.002,
        ])
        self.meat_hi = np.array([
            float(self.pan_center[0]) + 0.5 * self.meat_length,
            float(self.pan_center[1]) + 0.5 * self.meat_width,
            self.pan_top_z + 0.002 + self.meat_height,
        ])
        ...
        cut_z = self.pan_top_z - 0.018 + 2.0 * BLADE_HZ
        cut_y_lo = float(self.pan_center[1]) - 0.5 * self.meat_width - 0.015
        cut_y_hi = float(self.pan_center[1]) + 0.5 * self.meat_width + 0.015
        ...
        self.right_tf = self._waic_transform(RIGHT_HOME_TF)
        self.left_tf = self._waic_transform(LEFT_HOLD_TF)
        self._knife_prev_pos = wp.transform_get_translation(
            self._waic_transform(wp.transform(KNIFE_POS, wp.quat_identity()))
        )
    ...
    def _add_knife(self, builder: newton.ModelBuilder) -> int:
        ...
        self.knife_body = builder.add_body(
            xform=self._waic_transform(wp.transform(KNIFE_POS, wp.quat_identity())),
            label="knife_body",
        )
    ...
    def _build_motion_segments(self):
        ...
        knife_rest_tf = self._waic_transform(RIGHT_HOME_TF)
        knife_lift_tf = self._waic_transform(RIGHT_LIFT_TF)
        knife_hold_tf = self._waic_transform(RIGHT_HOLD_TF)
        return (
            ...
            (1.8, self.right_descend_tf, self.right_cut_end_tf, 1.0, 1.0, 1.0, 1.0),
    ...
    def _update_knife_transform(self, grasp_alpha: float) -> None:
        ...
        rest_tf = self._waic_transform(wp.transform(KNIFE_POS, wp.quat_identity()))
