...
"""Water pouring from one cup into another using MPM.

A source cup loaded from USD is filled with water particles, then tips
over to pour into a second (static) cup below and to the side.

Command: python -m newton.examples mpm_cup_to_cup
"""
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.src_cup_base = args.src_cup_base
        self.dst_cup_base = args.dst_cup_base
        self.cup_spacing = args.cup_spacing
        ...
        self.cup_mesh = newton.usd.get_mesh(usd_stage.GetPrimAtPath("/root/柱体"))

        self.build_cups(builder, args)
        ...
        self.src_cup_body_index = 0

        self.src_cup_pivot = wp.vec3(-self.cup_spacing / 2.0, 0.0, self.src_cup_base + self.cup_scale)

        self.viewer.show_particles = True
        self.viewer.set_model(self.model)
        cup_h = 2.0 * self.cup_scale
        mid_z = (self.src_cup_base + self.dst_cup_base) / 2.0 + cup_h
        self.viewer.set_camera(
            pos=wp.vec3(self.cup_spacing * 2.0, -self.cup_spacing * 3.0, mid_z + 0.15),
            pitch=-35.0,
            yaw=150.0,
        )

    def build_cups(self, builder, args):
        cup_scale = self.cup_scale
        mu = args.wall_friction
        spacing = self.cup_spacing

        src_cx = -spacing / 2.0
        src_cz = self.src_cup_base + cup_scale

        src_body = builder.add_body(
            xform=wp.transform(wp.vec3(src_cx, 0.0, src_cz), wp.quat_identity()),
            is_kinematic=True,
            label="src_cup",
        )
        ...
        dst_cx = spacing / 2.0
        dst_cz = self.dst_cup_base + cup_scale

        dst_body = builder.add_body(
            xform=wp.transform(wp.vec3(dst_cx, 0.0, dst_cz), wp.quat_identity()),
            is_kinematic=True,
            label="dst_cup",
        )
        builder.add_shape_mesh(
    ...
    def emit_particles(self, builder, args):
        ...
        src_cx = -self.cup_spacing / 2.0
        src_base = self.src_cup_base

        fill_lo_z = src_base + margin
        fill_hi_z = src_base + 2.0 * cup_scale - margin
        ...
        for r in r_vals:
            for t in theta_vals:
                for z in z_vals:
                    x = src_cx + r * np.cos(t)
                    y = r * np.sin(t)
                    if (x - src_cx) ** 2 + y * y <= emit_r * emit_r:
                        points.append([x, y, z])
    ...
    def create_parser():
        ...
        parser.add_argument("--src-cup-base", type=float, default=0.2, help="Height of source cup bottom above ground [m]")
        parser.add_argument("--dst-cup-base", type=float, default=0.0, help="Height of destination cup bottom above ground [m]")
        parser.add_argument("--cup-spacing", type=float, default=0.10, help="Horizontal distance between cup centers [m]")
        parser.add_argument("--tilt-start", type=float, default=0.05, help="Time [s] before the source cup starts tilting")
        parser.add_argument("--tilt-speed", type=float, default=2.0, help="Tilt angular speed [rad/s]")
        parser.add_argument("--max-tilt", type=float, default=1.8, help="Maximum tilt angle [rad]")
