...
"""Place a tablecloth with Dexforce W1 and SolverMuJoCoVBD in the WAIC house.

The robot's fixed arm and free-base motion are solved once in an *IK-only*
model, then replayed as cached targets in the independent MuJoCo/VBD scene. The
selected robot articulation is consumed as a prescribed kinematic collider,
while the private VBD path owns the cloth. The optional WAIC house USD is
rendering-only; the aligned table box is the sole tablecloth particle-contact
collider.

Run, from the repository root::

    uv run --extra examples -m newton.examples \
        mujoco_vbd_dexforce_bimanual_place_tablecloth_waic_house
"""
...
RADIUS = 0.010
SOFT_MARGIN = 0.020
CLOTH_SPAWN_CLEARANCE = RADIUS + SOFT_MARGIN + 0.005
CLOTH_CENTER = wp.vec3(
    float(OLD_TABLE_POS[0]) - 0.12,
    float(OLD_ROBOT_BASE_POS[1]),
    TABLE_TOP_Z + TABLE_COLLIDER_Z_OFFSET + CLOTH_SPAWN_CLEARANCE,
)
CLOTH_POS = wp.vec3(
...
# Fixed-joint collapsing attaches each hand-base collider to its terminal arm
# body, so include those bodies when switching the hand contact material.
HAND_CONTACT_KEYWORDS = ("hand", "thumb", "index", "middle", "ring", "pinky", "left_j7", "right_j7")
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        if self.solver.features.backend.value != "one_way_kinematic_soft":
            raise RuntimeError(
                "Tablecloth placement requires the one_way_kinematic_soft backend, "
                f"got {self.solver.features.backend.value}"
            )
    ...
    def _build_scene(self):
        ...
        if len(self.robot_articulations) != 1:
            raise RuntimeError(f"Expected one Dexforce W1 articulation, got {self.robot_articulations}")
        self.robot_shape_end = builder.shape_count
    ...
    def test_final(self):
        ...
        if self.frame_index < self.cached_joint_target_frame_count:
            return

        table_center = self._world_vec(OLD_TABLE_POS)
        expected_particle_z = (
            float(table_center[2]) + float(self.args.table_collider_z_offset) + TABLE_HALF_EXTENTS[2] + RADIUS
        )
        mean_particle_z = float(np.mean(particle_q[:, 2]))
        if abs(mean_particle_z - expected_particle_z) > 0.04:
            raise ValueError(
                "Tablecloth did not settle on the table after placement: "
                f"mean_z={mean_particle_z:.6f}, expected={expected_particle_z:.6f}"
            )
    ...
    def _configure_contact_materials(self):
        """Translate the reference VBD contact damping to the private VBD core's current units."""
        self.model.soft_contact_ke = LEGACY_SOFT_CONTACT_KE
        # The old fork used ``kd * ke`` as the normal damping coefficient;
        # the private VBD core stores that coefficient directly. Keep the particle
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            default=True,
            help="Render the aligned physics table; use --no-show-physics-table to hide it.",
        )
        parser.add_argument(
