...
DEFAULT_HOUSE_USD = "/home/oem/code/engine/newton/newton/examples/cloth/assets/house_background/House5_Simple2_visual_table01_table02_box_top_aligned_table02_w1_edge_translated.usd"
...
VBD_ITERATIONS = 20
IK_ITERATIONS = 24
ROBOT_CONTACT_KE = 900000.0
LEGACY_SOFT_CONTACT_KD = 0.05
LEGACY_SHAPE_CONTACT_KD = 1e-06
FULL_PROCESS_CLOTH_CONTACT_MU = 1.2
FULL_PROCESS_TABLE_CONTACT_MU = 1.2
P = wp.vec3
Q = wp.quat
LEFT_APPROACH = wp.transform(P(0.4117, 0.5145, 1.3642), Q(0.1099, 0.6989, -0.698, -0.1107))
...
RIGHT_LIFT = wp.transform(P(0.3764, -0.1638, 1.3433), Q(-0.0738, 0.703, 0.7037, -0.0721))
LEFT_TRAVEL = wp.transform(P(0.5542, 0.1679, 1.3376), Q(0.1093, 0.6986, -0.6984, -0.1107))
RIGHT_TRAVEL = wp.transform(P(0.6038, -0.1635, 1.3444), Q(-0.0738, 0.703, 0.7036, -0.0719))
...
LEFT_2GRASP = wp.transform(P(0.6225, 0.2642, 1.246), Q(0.0216, 0.7067, -0.7067, -0.0245))
...
LEFT_2TRAVEL = wp.transform(P(0.6436, 0.0678, 1.3139), Q(-0.0212, -0.7067, 0.7067, 0.025))
...
RIGHT_2PLACE = wp.transform(P(0.6227, -0.086, 1.2694), Q(0.0207, 0.7001, 0.7123, -0.0458))
LEFT_2RELEASE = wp.transform(P(0.6438, 0.0677, 1.4336), Q(-0.0212, -0.7067, 0.7067, 0.0251))
RIGHT_2RELEASE = wp.transform(P(0.6227, -0.086, 1.4101), Q(0.0207, 0.7001, 0.7123, -0.0458))
...
class Example:
    ...
    def _build_scene(self):
        ...
        builder.default_shape_cfg.ke = 500000.0
        builder.default_shape_cfg.kd = 1e-06
        ...
        self.model.soft_contact_ke = 300000.0
        ...
        self._set_materials(0.5, 0.25, 0.5)

    def _add_table(self, builder):
        old_table = self._world_vec(OLD_TABLE_POS)
        cfg = newton.ModelBuilder.ShapeConfig(
            ke=500000.0, kd=1e-06, mu=1.2, is_visible=bool(self.args.show_physics_table)
        )
    ...
    def _solve_runtime_ik_frame(self):
        ...
        wp.launch(
            _lock_q, self.lock_indices.shape[0], [self.ik_q, self.lock_indices, self.lock_values], device=self.device
        )
        wp.launch(_copy_joint_q, self.model.joint_coord_count, [self.ik_q[0], self.frame_q_end], device=self.device)
        wp.launch(
    ...
    def _joint_limits(self):
        ...
        for j, label in enumerate(self.model.joint_label):
            if label not in controlled:
                lo[int(qds[j])] = q[int(qs[j])] - 0.0001
                hi[int(qds[j])] = q[int(qs[j])] + 0.0001
        return (lo, hi)
    ...
    def _locked_q(self):
        ...
        return (
            ...
            wp.array([q[i] for i in ids], dtype=wp.float32, device=self.device),
        )
    ...
    def _hand_q(self):
        ...
        open_q = []
        grasp = []
        targets = {"HAND_THUMB2": 0.84, "HAND_THUMB1": 0.46, "HAND_INDEX": 0.7, "INDEX_PIP": 0.9}
...
def main():
    """Run the standalone bimanual T-shirt folding demo."""
