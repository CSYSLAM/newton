...
ARMADILLO_K_MU = 500000.0
ARMADILLO_K_LAMBDA = 5000000.0
ARMADILLO_K_DAMP = 1e-07
ARMADILLO_PARTICLE_RADIUS = 0.005
ROLLER_INNER_RADIUS = 0.36 * CRUSHER_SCENE_SCALE
ROLLER_OUTER_RADIUS = 0.4 * CRUSHER_SCENE_SCALE
ROLLER_LENGTH = 1.6 * CRUSHER_SCENE_SCALE
ROLLER_TEETH = 16
...
CRUSHER_CONTACT_KE = 1000000.0
CRUSHER_CONTACT_KD = 1e-07
CRUSHER_CONTACT_MU = 0.2
HAND_CONTACT_KE = 5000000.0
...
PREGRASP_HOLD_DURATION = 0.5
GRASP_SETTLE_DURATION = 0.75
TRANSFER_HOLD_DURATION = 0.3
DROP_SETTLE_DURATION = 3.0
SCRIPTED_LIFT_HEIGHT = 0.13
CAMERA_POS = wp.vec3(0.95, -4.05, 1.72)
CAMERA_PITCH = -15.0
CAMERA_YAW = 128.0
CAMERA_FOV = 46.0
GEAR_COLOR = (0.5, 0.53, 0.58)
ARMADILLO_COLOR = (0.2, 0.78, 0.42)
TABLE_COLOR = (0.35, 0.42, 0.48)
GROUND_COLOR = (0.18, 0.2, 0.23)
HAND_JOINTS = (
    "RIGHT_HAND_THUMB1",
    "RIGHT_HAND_THUMB2",
    "RIGHT_HAND_INDEX",
...
def _translated_transform(transform: wp.transform, offset: wp.vec3) -> wp.transform:
    ...
    return wp.transform(wp.transform_get_translation(transform) + offset, wp.transform_get_rotation(transform))
...
def _transform_duration(
    start: wp.transform, end: wp.transform, translation_speed: float = HAND_TRANSLATION_SPEED
) -> float:
    """Choose a hand-motion duration from its translation distance."""
    start_position = np.asarray(wp.transform_get_translation(start), dtype=np.float64)
    end_position = np.asarray(wp.transform_get_translation(end), dtype=np.float64)
    return max(MINIMUM_ROOT_PHASE_DURATION, float(np.linalg.norm(end_position - start_position)) / translation_speed)
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.soft_contact_material_index = wp.full(1, _CONTACT_MATERIAL_APPROACH, dtype=wp.int32, device=self.device)
    ...
    def _build_scene(self, asset_path: Path):
        ...
        table_cfg = newton.ModelBuilder.ShapeConfig(
            density=0.0, ke=300000.0, kd=0.0001, mu=0.9, margin=0.0, is_visible=True
        )
        ...
        self.left_gear_shape = builder.add_shape_mesh(
            self.left_gear_body, mesh=gear_mesh, cfg=gear_cfg, label="left_crusher_gear_mesh"
        )
        self.right_gear_shape = builder.add_shape_mesh(
            self.right_gear_body, mesh=gear_mesh, cfg=gear_cfg, label="right_crusher_gear_mesh"
        )
        builder.add_shape_collision_filter_pair(self.left_gear_shape, self.right_gear_shape)
        ground_cfg = newton.ModelBuilder.ShapeConfig(
            density=0.0, ke=CRUSHER_CONTACT_KE, kd=CRUSHER_CONTACT_KD, mu=CRUSHER_CONTACT_MU
        )
        builder.add_ground_plane(height=GROUND_HEIGHT, cfg=ground_cfg, color=GROUND_COLOR, label="crusher_ground")
    ...
    def _build_phases(self) -> tuple[_Phase, ...]:
        ...
        transfer_offset = wp.vec3(CRUSHER_CENTER_X - ARMADILLO_CENTER_X, CRUSHER_CENTER_Y - ARMADILLO_CENTER_Y, 0.0)
        ...
        grasp_duration = max(
            finger_duration, _transform_duration(approach_root, grasp_root, HAND_GRASP_TRANSLATION_SPEED)
        )
        return (
            ...
            _Phase("pregrasp_hold", PREGRASP_HOLD_DURATION, approach_root, approach_root, OPEN_JOINTS, OPEN_JOINTS),
            _Phase("close", grasp_duration, approach_root, grasp_root, OPEN_JOINTS, GRASP_JOINTS),
            _Phase("grasp_settle", GRASP_SETTLE_DURATION, grasp_root, grasp_root, GRASP_JOINTS, GRASP_JOINTS),
            _Phase(
                "lift",
            ...
            _Phase("transfer_hold", TRANSFER_HOLD_DURATION, transfer_root, transfer_root, GRASP_JOINTS, GRASP_JOINTS),
    ...
    def _sample(self, time_s: float) -> tuple[wp.transform, dict[str, float], int]:
        ...
        for phase_index, phase in enumerate(self.phases):
            if time_s <= phase.duration:
                ...
                return (_interpolate_transform(phase.root_start, phase.root_end, alpha), finger_target, phase_index)
    ...
    def _enter_phase(self, phase_index: int):
        ...
        if phase.name == "close":
            self._set_contact_material(_CONTACT_MATERIAL_GRASP, HAND_GRASP_FRICTION, HAND_GRASP_SOFT_CONTACT_MU)
        if phase.name == "lift":
        ...
        if phase.name == "release":
            ...
            self._set_contact_material(_CONTACT_MATERIAL_RELEASE, HAND_RELEASE_FRICTION, CRUSHER_CONTACT_MU)
    ...
    def _simulate_substeps(self):
        ...
        for substep in range(self.sim_substeps):
            ...
            wp.launch(_advance_crusher_time, dim=1, inputs=[self.crusher_time, self.sim_dt], device=self.device)
    ...
    def create_parser():
        ...
        parser.add_argument("--asset-path", type=Path, default=ARMADILLO_ASSET, help="Armadillo legacy-VTK asset path.")
        parser.add_argument(
            "--rotation-speed",
            type=float,
        ...
        parser.add_argument("--substeps", type=int, default=SIM_SUBSTEPS, help="Physics substeps per displayed frame.")
        parser.add_argument(
            "--vbd-iterations", type=int, default=VBD_ITERATIONS, help="VBD iterations per physics substep."
        )
        parser.add_argument(
            "--graph-capture",
...
def main():
    """Run the standalone right-hand Armadillo crusher demo."""
