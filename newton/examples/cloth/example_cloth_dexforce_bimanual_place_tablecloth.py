...
class Example:
    def __init__(self, viewer, args):
        ...
        self.table_shape_end = self.table_shape_start + 1
    ...
    def _set_hand_contact_mu(self, hand_mu: float) -> None:
        if self.hand_shape_indices.size == 0:
            return
        shape_mu = self.model.shape_material_mu.numpy()
        shape_mu[self.hand_shape_indices] = hand_mu
        self.model.shape_material_mu = wp.array(
            shape_mu, dtype=self.model.shape_material_mu.dtype, device=self.model.shape_material_mu.device
        )
    ...
    def _motion_phase(self) -> str:
        ...
        if base_time < ROOT_PRE_APPROACH_TIME:
            return "turn_to_table"
        if base_time < ROOT_PRE_APPROACH_TIME + PRE_LAYDOWN_TIME:
            return "pre_laydown"
        if base_time < ROOT_MOTION_DURATION:
            return "lay_down"
        if base_time < ROOT_MOTION_DURATION + RELEASE_TIME:
            return "release_hold"
        if base_time < ROOT_MOTION_DURATION + RELEASE_TIME + POST_RELEASE_HOLD_TIME:
    ...
    def _prepare_frame_targets(self) -> None:
        ...
        self._apply_root_motion(self.frame_joint_q_end, self.sim_time + self.frame_dt)
    ...
    def simulate(self) -> None:
        ...
        for substep in range(self.sim_substeps):
            ...
            self._apply_root_motion(self.state_1.joint_q, self.sim_time + (substep + 1) * self.sim_dt)
            wp.launch(
