...
###########################################################################
# Example Cloth Dexforce Bimanual IK T-Shirt (two-finger gripper)
#
# Same T-shirt IK debugging scene as ``cloth_dexforce_bimanual_ik_tshirt``
# but built on the ``DexforceW1V021-simple`` two-finger gripper URDF. The
# gripper has a single driven prismatic finger per hand (the second finger
# mimics it), so the hand target is just that one prismatic joint.
#
# Command: python -m newton.examples cloth_dexforce_bimanual_ik_tshirt_simple
#
###########################################################################
...
# TCP offset along the wrist link. Reused from the dexterous-hand scene; the
# two-finger gripper has slightly different geometry so this is the knob to
# retune if a captured trajectory needs re-targeting.
TCP_OFFSET = wp.vec3(-0.18, 0.0, 0.0)

# Prismatic finger joint value when the gripper is "open". Prismatic range is
# [0, 0.05]; q=0 fully closes the two fingers, q=0.05 opens them to a ~0.10 m
# gap. Lower this to make the open grip tighter.
OPEN_FINGER_GAP = 0.05
...
class Example:
    def __init__(self, viewer, args):
        ...
        urdf_path = Path("E:/csy_work/CG/assets/DexforceW1V021-simple") / "DexforceW1V021.urdf"
        builder.add_urdf(
            urdf_path,
    ...
    def _build_hand_targets(self) -> tuple[wp.array, wp.array, wp.array]:
        ...
        open_values = []
        grasp_values = []
        # Two-finger gripper prismatic joints: q=0 fully closed (fingers touch),
        # q increases => fingers move apart (q=0.05 gives ~0.10 m gap).
        #   open  = OPEN_FINGER_GAP  (fingers wide apart, alpha=0)
        #   grasp = 0.0              (fingers together,        alpha=1)
        # FINGER2 mimics FINGER1 (multiplier=1, offset=0), so drive both to the
        # same value to stay consistent with the mimic constraint.
        targets = {
            "FINGER1_JOINT": OPEN_FINGER_GAP,
            "FINGER2_JOINT": OPEN_FINGER_GAP,
        }
        ...
        for side in ("LEFT", "RIGHT"):
            for suffix in self.HAND_JOINT_SUFFIXES:
                ...
                q_idx = int(q_start[joint_idx])
                # home pose has q=0 (closed); the open target is the large value.
                open_value = float(targets.get(suffix, q_home[q_idx]))
                q_indices.append(q_idx)
                open_values.append(open_value)
                grasp_values.append(0.0)
    ...
    def _prepare_frame_targets(self) -> None:
        ...
        # In the IK scene the gripper is held closed so the T-shirt can be
        # manipulated interactively when dragging the gizmos.
        wp.launch(
    ...
    # Two-finger gripper: one driven prismatic joint per hand (FINGER2 mimics
    # FINGER1). These are also registered as controlled so the IK scene can
    # overwrite their targets via set_indexed_joint_q_kernel.
    HAND_JOINT_SUFFIXES = (
        "FINGER1_JOINT",
        "FINGER2_JOINT",
    )
