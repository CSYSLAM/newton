...
###########################################################################
# Example Cloth Dexforce Bimanual Grasp Cloth V4
#
# Uses the same Dexforce/table/large-cloth scene as the interactive bimanual
# IK cloth example, then drives the floating W1 root backward and through a
# yaw turn after both hands grasp and lift the cloth.
#
# Command: python -m newton.examples cloth_dexforce_bimanual_grasp_cloth_v4
#
###########################################################################
...
def set_free_root_motion_kernel(
    ...
    base_time: float,
    move_back_distance: float,
    move_back_time: float,
    rotate_radians: float,
    rotate_time: float,
    joint_q_out: wp.array[wp.float32],
):
    move_u = smoothstep(base_time / wp.max(move_back_time, 1.0e-6))
    rotate_u = smoothstep((base_time - move_back_time) / wp.max(rotate_time, 1.0e-6))
    yaw = rotate_radians * rotate_u
    q_yaw = wp.quat(0.0, 0.0, wp.sin(0.5 * yaw), wp.cos(0.5 * yaw))
    q0 = wp.quat(root_q0[3], root_q0[4], root_q0[5], root_q0[6])
    q = q_yaw * q0

    joint_q_out[root_q_start + 0] = root_q0[0] - move_back_distance * move_u
    joint_q_out[root_q_start + 1] = root_q0[1]
...
MOVE_BACK_DISTANCE = 0.35
MOVE_BACK_TIME = 1.5
ROTATE_DEGREES = 90.0
ROTATE_TIME = 3.0
FINAL_HOLD_TIME = 1.0
SCRIPT_DURATION = 0.8 + 2.0 + 2.0 + 2.0 + 3.0 + 2.0
DEFAULT_NUM_FRAMES = int(math.ceil((SCRIPT_DURATION + MOVE_BACK_TIME + ROTATE_TIME + FINAL_HOLD_TIME) * 60.0))
...
RIGHT_APPROACH_TF = wp.transform(
    wp.vec3(0.3363, -0.6980, 1.3628),
    wp.quat(0.0510, 0.7059, 0.7047, 0.0499),
)
RIGHT_GRASP_TF = wp.transform(
    wp.vec3(0.5897, -0.4651, 1.2055),
    wp.quat(0.0245, 0.6878, 0.7139, -0.1294),
)
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.lift_done_time = SCRIPT_DURATION
        ...
        self.left_approach_tf = self._mirror_y_transform(RIGHT_APPROACH_TF)
        ...
        self.left_grasp_tf = self._mirror_y_transform(self.right_grasp_tf)
    ...
    def _build_motion_segments(self) -> tuple[tuple[float, wp.transform, wp.transform, wp.transform, wp.transform, float, float], ...]:
        return (
            ...
            (2.0, self.left_home_tf, self.left_approach_tf, self.right_home_tf, self.right_approach_tf, 0.0, 0.0),
            (2.0, self.left_approach_tf, self.left_grasp_tf, self.right_approach_tf, self.right_grasp_tf, 0.0, 0.0),
            (2.0, self.left_grasp_tf, self.left_grasp_tf, self.right_grasp_tf, self.right_grasp_tf, 0.0, 1.0),
            (3.0, self.left_grasp_tf, self.left_lift_tf, self.right_grasp_tf, self.right_lift_tf, 1.0, 1.0),
            (2.0, self.left_lift_tf, self.left_lift_tf, self.right_lift_tf, self.right_lift_tf, 1.0, 1.0),
        )
    ...
    def _base_motion_time(self, query_time: float) -> float:
        return max(float(query_time) - self.lift_done_time, 0.0)

    def _motion_phase(self) -> str:
        base_time = self.sim_time - self.lift_done_time
        if base_time < 0.0:
            return 'lift'
        if base_time < MOVE_BACK_TIME:
            return 'move_back'
        if base_time < MOVE_BACK_TIME + ROTATE_TIME:
            return 'rotate'
        return 'final_hold'
