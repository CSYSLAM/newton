...
###########################################################################
# Example Cloth Franka MJVBD Interactive
#
# Same shirt/table/Franka setup as the MuJoCo shirt example but with
...
#
# Command: python -m newton.examples.cloth.example_cloth_franka_mjvbd_shirt
#          python -m newton.examples.cloth.example_cloth_franka_mjvbd_shirt --disable-franka
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.table_viz_color = wp.array([wp.vec3(0.16, 0.55, 0.78)], dtype=wp.vec3)
    ...
    def _prepare_substep_state(self) -> None:
        ...
        # Full State.assign() also copies forces, velocities, and namespaced arrays
        # that are either cleared or recomputed in this example's hot path.
    ...
    def create_articulation(self, builder: ModelBuilder) -> None:
        ...
        self.robot_key_poses = np.array(
            [
                [3.0, 22.0, -60.0, 40.0, *grasp_quat, gripper_open],
                [1.0, 25.0, -54.0, 7.0, *grasp_quat, gripper_open],
                [1.0, 25.0, -54.0, 7.0, *grasp_quat, gripper_close],
                [1.0, 22.0, -60.0, 16.0, *grasp_quat, gripper_close],
                [1.0, 12.0, -60.0, 28.0, *grasp_quat, gripper_close],
                [1.0, -6.0, -60.0, 28.0, *grasp_quat, gripper_close],
                [1.0, -16.0, -60.0, 28.0, *grasp_quat, gripper_close],
                [1.0, -24.0, -60.0, 17.0, *grasp_quat, gripper_close],
                [1.0, -30.0, -60.0, 10.0, *grasp_quat, gripper_close],
                [1.0, -30.0, -60.0, 10.0, *grasp_quat, gripper_open],
                [1.0, -30.0, -60.0, 30.0, *grasp_quat, gripper_open],

                [3.0, -5.0545, -15.7923, 30.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_open],
                [1.0, -5.0545, -15.7923, 20.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_open],
                [1.0, -5.0545, -15.7923, 7.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_open],
                [1.0, -5.0545, -15.7923, 7.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_close],
                [1.0, -5.0545, -15.7923, 20.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_open],
                [1.0, -5.0545, -15.7923, 30.0000, 0.8787, -0.4669, -0.0971, 0.0215, gripper_open],
            ],
            dtype=np.float32,
        )
    ...
    def render(self) -> None:
        ...
        target1 = wp.vec3(-26.0, -60.0, 17.0)
        cube_pos_m = wp.vec3(
