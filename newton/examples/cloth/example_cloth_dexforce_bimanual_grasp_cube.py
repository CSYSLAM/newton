...
###########################################################################
# Example Cloth Dexforce Bimanual Grasp Cube
#
# Loads DexforceW1V021 in the same table scene as the bimanual IK grasp
# example, drives both TCPs through mirrored recorded IK poses, closes both
# hands around the two cubes, then lifts both cubes.
#
# Command: python -m newton.examples cloth_dexforce_bimanual_grasp_cube
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.max_left_cube_height = float(LEFT_CUBE_POS[2])
        self.max_right_cube_height = float(RIGHT_CUBE_POS[2])
        ...
        self.hand_indices, self.hand_open, self.hand_grasp = self._build_hand_targets()
        self.setup_ik()
        wp.launch(
    ...
    def _add_table_scene(self, builder: newton.ModelBuilder) -> None:
        ...
        self.left_cube_body = builder.add_body(
            xform=wp.transform(LEFT_CUBE_POS, self._mirror_y_quat(RIGHT_CUBE_ROT)),
            label="left_grasp_cube",
        )
        builder.add_shape_box(
            body=self.left_cube_body,
        ...
        self.right_cube_body = builder.add_body(
            xform=wp.transform(RIGHT_CUBE_POS, RIGHT_CUBE_ROT),
            label="right_grasp_cube",
        )
        builder.add_shape_box(
            body=self.right_cube_body,
            xform=wp.transform_identity(),
            hx=CUBE_HALF,
            hy=CUBE_HALF,
            hz=CUBE_HALF,
    ...
    def _build_motion_segments(self) -> tuple[
        ...
        left_approach_tf = self._mirror_y_transform(RIGHT_APPROACH_TF)
        left_clear_tf = self._mirror_y_transform(RIGHT_CLEAR_TF)
        left_grasp_tf = self._mirror_y_transform(RIGHT_GRASP_TF)
        right_lift_tf = self._offset_transform(RIGHT_GRASP_TF, wp.vec3(0.0, 0.0, LIFT_HEIGHT))
        left_lift_tf = self._mirror_y_transform(right_lift_tf)

        return (
            (0.8, self.left_home_tf, self.left_home_tf, self.right_home_tf, self.right_home_tf, 0.0, 0.0),
            (2.0, self.left_home_tf, left_approach_tf, self.right_home_tf, RIGHT_APPROACH_TF, 0.0, 0.0),
            (2.0, left_approach_tf, left_clear_tf, RIGHT_APPROACH_TF, RIGHT_CLEAR_TF, 0.0, 0.0),
            (2.0, left_clear_tf, left_grasp_tf, RIGHT_CLEAR_TF, RIGHT_GRASP_TF, 0.0, 0.0),
            (1.2, left_grasp_tf, left_grasp_tf, RIGHT_GRASP_TF, RIGHT_GRASP_TF, 0.0, 1.0),
            (2.2, left_grasp_tf, left_lift_tf, RIGHT_GRASP_TF, right_lift_tf, 1.0, 1.0),
            (1.2, left_lift_tf, left_lift_tf, right_lift_tf, right_lift_tf, 1.0, 1.0),
        )
    ...
    def _build_hand_targets(self) -> tuple[wp.array, wp.array, wp.array]:
        ...
        for side in ("LEFT", "RIGHT"):
            for suffix, target in grasp_targets.items():
    ...
    def _sample_script(self) -> tuple[wp.transform, wp.transform, float]:
        ...
        for duration, left_start, left_end, right_start, right_end, start_grasp, end_grasp in self.motion_segments:
            if remaining <= duration:
                ...
                grasp_alpha = start_grasp * (1.0 - alpha) + end_grasp * alpha
                return left_tf, right_tf, grasp_alpha
            remaining -= duration

        _, _, left_end, _, right_end, _, end_grasp = self.motion_segments[-1]
        return left_end, right_end, end_grasp
    ...
    def _update_control_targets(self) -> None:
        ...
        wp.launch(
            ...
            dim=self.hand_indices.shape[0],
            inputs=[self.hand_indices, self.hand_open, self.hand_grasp, grasp_alpha],
            outputs=[self.control.joint_target_pos],
            device=self.model.device,
        )
    ...
    def test_post_step(self) -> None:
        ...
        self.max_left_cube_height = max(self.max_left_cube_height, float(body_q[self.left_cube_body, 2]))
        self.max_right_cube_height = max(self.max_right_cube_height, float(body_q[self.right_cube_body, 2]))

    def test_final(self) -> None:
        body_q = self.state_0.body_q.numpy()
        for label, body_idx, y_min, y_max in (
            ("left cube", self.left_cube_body, 0.2, 1.0),
            ("right cube", self.right_cube_body, -1.0, -0.2),
        ):
            cube_pos = body_q[body_idx, :3]
            if not np.all(np.isfinite(cube_pos)):
                raise ValueError(f"{label} position is not finite: {cube_pos}")
            if not (-0.1 < cube_pos[0] < 0.8 and y_min < cube_pos[1] < y_max and 1.0 < cube_pos[2] < 1.8):
                raise ValueError(f"{label} ended outside the expected grasp volume: {cube_pos}")
    ...
    def _mirror_y_transform(self, tf: wp.transform) -> wp.transform:
        ...
        return wp.transform(
            ...
            self._mirror_y_quat(quat),
        )

    def _mirror_y_quat(self, quat: wp.quat) -> wp.quat:
