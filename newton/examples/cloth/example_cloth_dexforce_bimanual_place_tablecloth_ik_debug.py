...
###########################################################################
# Example Cloth Dexforce Bimanual Place Tablecloth IK Debug
#
# Loads DexforceW1V021 in its initial arm pose, closes the thumb and index
# fingers, and exposes an IK gizmo for each wrist TCP so the tablecloth
# grasp trajectory can be debugged interactively.
#
# Command: python -m newton.examples cloth_dexforce_bimanual_place_tablecloth_ik_debug
#
###########################################################################
...
FINGER_CONTACT_KEYWORDS = ("thumb", "index", "middle", "ring", "pinky")

RIGHT_THUMB_TIP_LINK = "right_thumb_dist"
RIGHT_INDEX_TIP_LINK = "right_index_dist"

RIGHT_TCP_ROT = wp.quat(0.0245, 0.6878, 0.7139, -0.1294)
RIGHT_GRASP_TF = wp.transform(
    wp.vec3(float(CLOTH_CENTER[0]) + 0.5 * CLOTH_DIM_X * CLOTH_CELL_X - 0.012, TABLECLOTH_START_Y - 0.5 * CLOTH_DIM_Y * CLOTH_CELL_Y + 0.012, TABLE_TOP_Z + 0.045),
    RIGHT_TCP_ROT,
)
HAND_CENTER_CUBE_HALF_EXTENTS = 0.012
HAND_CENTER_CUBE_COLOR = (0.95, 0.85, 0.2)
HAND_CENTER_CUBE_MESH = "/tablecloth_ik_debug_finger_center_mesh"
HAND_CENTER_CUBE_INSTANCES = "/tablecloth_ik_debug_finger_center_instances"

APPROACH_TIME = 1.4
CLOSE_START_ALPHA = 0.35
CLOSE_END_ALPHA = 1.0
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.finger_center_tf = self._right_finger_center_transform()
        self.right_approach_tf = self.right_tf
        self.right_grasp_tf = self.right_tf
        ...
        self.viewer.set_camera(wp.vec3(1.10, -2.35, 1.70), -16.0, 105.0)
        self.viewer.log_geo(
            HAND_CENTER_CUBE_MESH,
            newton.GeoType.BOX,
            (HAND_CENTER_CUBE_HALF_EXTENTS, HAND_CENTER_CUBE_HALF_EXTENTS, HAND_CENTER_CUBE_HALF_EXTENTS),
            0.0,
            True,
            hidden=False,
        )
    ...
    def _configure_robot(self, builder: newton.ModelBuilder) -> None:
        ...
        for side in ("LEFT", "RIGHT"):
            for suffix, target in HAND_CLOSED_TARGETS.items():
                joint_idx = self._builder_joint_index(builder, f"{side}_{suffix}")
                q_idx = builder.joint_q_start[joint_idx]
                dof_idx = builder.joint_qd_start[joint_idx]
                builder.joint_q[q_idx] = target
                builder.joint_target_pos[dof_idx] = target
    ...
    def _add_cloth(self, builder: newton.ModelBuilder) -> None:
        builder.add_cloth_grid(
            ...
            label="tablecloth_ik_debug_cloth",
        )
    ...
    def _configure_particle_contacts(self) -> None:
        ...
        for shape_idx in range(self.robot_shape_end):
            ...
            if body_idx < 0:
                continue
            label = self.model.body_label[body_idx].lower()
            if any(keyword in label for keyword in FINGER_CONTACT_KEYWORDS):
                flags[shape_idx] &= ~collide_particles
    ...
    def _build_hand_targets(self) -> tuple[wp.array, wp.array, wp.array]:
        ...
        for side in ("LEFT", "RIGHT"):
            for suffix, target in HAND_CLOSED_TARGETS.items():
                joint_idx = self._builder_joint_index(self.model_builder, f"{side}_{suffix}") if False else None
    ...
    def _grasp_alpha(self) -> float:
        ...
        if self.sim_time < 2.0:
            return float(np.clip((self.sim_time - 1.0) / 1.0, CLOSE_START_ALPHA, CLOSE_END_ALPHA))
        return 1.0
    ...
    def render(self) -> None:
        ...
        finger_center_tf = self._right_finger_center_transform()
        self.viewer.log_instances(
            HAND_CENTER_CUBE_INSTANCES,
            HAND_CENTER_CUBE_MESH,
            wp.array([finger_center_tf], dtype=wp.transform, device=self.model.device),
            None,
            wp.array([wp.vec3(*HAND_CENTER_CUBE_COLOR)], dtype=wp.vec3, device=self.model.device),
            None,
            hidden=False,
        )
        self.viewer.log_state(self.state_0)
        self.viewer.log_mesh(
            "/tablecloth_ik_debug_cloth",
    ...
    def _right_finger_center_transform(self) -> wp.transform:
        thumb_tf = self._current_link_transform(RIGHT_THUMB_TIP_LINK)
        index_tf = self._current_link_transform(RIGHT_INDEX_TIP_LINK)
        thumb_pos = wp.transform_get_translation(thumb_tf)
        index_pos = wp.transform_get_translation(index_tf)
        center_pos = wp.vec3(
            0.5 * (float(thumb_pos[0]) + float(index_pos[0])),
            0.5 * (float(thumb_pos[1]) + float(index_pos[1])),
            0.5 * (float(thumb_pos[2]) + float(index_pos[2])),
        )
        return wp.transform(center_pos, wp.quat_identity())

    def _current_link_transform(self, link_name: str) -> wp.transform:
        body_idx = self._body_index(link_name)
        return self._current_tcp_transform(body_idx, wp.vec3(0.0, 0.0, 0.0))
