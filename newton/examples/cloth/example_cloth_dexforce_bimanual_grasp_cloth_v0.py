...
###########################################################################
# Example Cloth Dexforce Bimanual Grasp Cloth
#
# Uses the same Dexforce/table/large-cloth scene as the interactive bimanual
# IK cloth example, but drives both wrist TCPs through a mirrored scripted
# trajectory to test whether the cloth can be grasped and lifted.
#
# Command: python -m newton.examples cloth_dexforce_bimanual_grasp_cloth
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.left_grasp_tf = self._mirror_y_transform(RIGHT_GRASP_TF)
        self.right_grasp_tf = RIGHT_GRASP_TF
    ...
    def _build_motion_segments(self) -> tuple[tuple[float, wp.transform, wp.transform, wp.transform, wp.transform, float, float], ...]:
        return (
            ...
            (1.2, self.left_grasp_tf, self.left_grasp_tf, self.right_grasp_tf, self.right_grasp_tf, 0.0, 1.0),
            (2.4, self.left_grasp_tf, self.left_lift_tf, self.right_grasp_tf, self.right_lift_tf, 1.0, 1.0),
