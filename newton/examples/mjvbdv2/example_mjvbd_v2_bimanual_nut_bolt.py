...
Run from the repository root::
    ...
    uv run --extra examples -m newton.examples \
        mjvbd_v2_bimanual_nut_bolt --viewer gl
"""
...
SIM_SUBSTEPS = 8
VBD_ITERATIONS = 40
# The M20 mesh bounds make 4.8 turns place the bolt tip flush with the nut's
# hand-facing outer face.  Another 0.75 mm of advance avoids coplanar end faces
# and visibly exposes the bolt tip from the first frame.
INITIAL_BOLT_TIP_PROTRUSION = 7.5e-4
...
LEFT_HAND_CONTACT_KE = 2.0e6
RIGHT_HAND_CONTACT_KE = 1.0e5
THREAD_CONTACT_KE = 3.0e5
THREAD_CONTACT_KD = 3.0e2
THREAD_CONTACT_GAP = 2.0e-4
CONTACT_KD = 10.0
LEFT_HAND_CONTACT_KD = 5.0e3
LEFT_HAND_CONTACT_MU = 1.0e2
RIGHT_HAND_CONTACT_MU = 2.0
RIGHT_MIDDLE_CONTACT_MU = 10.0
THREAD_CONTACT_MU = 1.0
NUT_ANGULAR_DAMPING = 3.0e-2
...
RIGHT_HAND_JOINT_OFFSETS_DEGREES = {
    "MIDDLE_PIP": -24.0,
}
...
RIGHT_SIDE_STROKE_END_ROOT = np.array(
    (0.0854, -0.1647, 0.6368 + WORKSPACE_HEIGHT_OFFSET),
    dtype=np.float32,
)
RIGHT_SIDE_STROKE_START_ROOT = np.array(
    (0.0854, -0.2200, 0.6368 + WORKSPACE_HEIGHT_OFFSET),
    dtype=np.float32,
)
...
MAX_BOLT_GRIP_TRANSLATION = 0.020
...
# A cylindrical power grasp adapted from the rigid-cube W1 example. Both hands
# in the full W1 use the same joint suffixes.
HAND_JOINT_DEGREES = {
    "HAND_THUMB1": 6.0,
    "HAND_THUMB2": 90.0,
    "HAND_INDEX": 41.0,
...
def _hexagonalize_bolt_head(vertices: np.ndarray) -> np.ndarray:
    ...
    outer_head = (vertices[:, 2] <= BOLT_HEAD_END + 1.0e-6) & (radii >= 0.95 * BOLT_HEAD_RADIUS)
...
def _apply_nut_angular_damping(
    ...
    wp.atomic_add(
        body_f,
        nut_body,
        wp.spatial_vector(wp.vec3(0.0), -damping * angular_velocity),
    )
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        for description, path in (
            ("Dexforce W1 URDF", robot_path),
            ("Bimanual hand keyframe", keyframe_path),
        ):
            if not path.is_file():
        ...
        self.bolt_body = builder.add_body(
            xform=wp.transform(wp.vec3(*bolt_position), bolt_rotation),
            label="left_hand_bolt",
        )
        ...
        # Full turns do not change the visible phase. A small positive phase
        # places the loose M20 flanks in their clearance instead of starting
        # the SDF pair with the thread crests interpenetrating.
        ...
        self.nut_body = builder.add_body(
            xform=wp.transform(wp.vec3(*nut_position), nut_rotation),
            label="right_hand_nut",
        )
        nut_hand_cfg = newton.ModelBuilder.ShapeConfig(
            density=8000.0,
            ke=RIGHT_HAND_CONTACT_KE,
            kd=CONTACT_KD,
            mu=RIGHT_HAND_CONTACT_MU,
            gap=0.0,
            margin=0.0,
        )
        ...
        self._thread_initial_relative_rotation_host = np.asarray(
            tuple(thread_initial_relative_rotation),
            dtype=np.float32,
        )
        ...
        for shape in (
            *self.hand_shapes["LEFT"],
            *self.hand_shapes["RIGHT"],
            self.nut_thread_shape,
        ):
            builder.shape_flags[shape] |= collide_shapes
        ...
        self.solver = newton.solvers.SolverMJVBDV2(
            ...
            vbd_options={
                ...
                "rigid_contact_stick_freeze_translation_eps": 1.0e-3,
                "rigid_contact_stick_freeze_angular_eps": 1.0e-3,
        ...
        if self.solver.features.backend != "vbd_kinematic_full":
            raise RuntimeError(
                f"The bimanual nut/bolt scene requires vbd_kinematic_full, got {self.solver.features.backend}"
            )
    ...
    def _set_robot_ik_targets(self, root_targets: dict[str, wp.transform]) -> None:
        ...
        self.right_rotation_objective.set_target_rotation(
            0,
            self._quaternion_vector(wp.transform_get_rotation(right)),
        )
    ...
    def _locked_robot_q(self) -> tuple[wp.array[int], wp.array[float]]:
        ...
        return wp.array(indices, dtype=wp.int32, device=self.model.device), wp.array(
            [q[index] for index in indices],
            dtype=wp.float32,
            device=self.model.device,
        )
    ...
    def _build_right_stroke_trajectory(self) -> None:
        ...
        self.right_root_waypoints.append((frame, stroke_start))
        # The nut is dynamic from frame zero. Hold radial pressure briefly so
        # the native fingertip establishes persistent contact before the first
        # full-angle sweep.
    ...
    def simulate(self) -> None:
        ...
        # The VBD backend may leave a small contact correction in the output
        # pose of a kinematic body. Restore the exact robot FK target exposed
        # to the viewer without touching the dynamic threaded pair.
        newton.eval_fk(
            self.model,
            self.state_0.joint_q,
            self.state_0.joint_qd,
    ...
    def _track_test_state(self) -> None:
        ...
        self.maximum_other_right_hand_nut_contact_count = max(
            self.maximum_other_right_hand_nut_contact_count,
            int(other_nut_contacts),
        )
        ...
        for stroke, (frame_start, frame_end) in enumerate(self.stroke_frame_ranges):
            if frame_start <= simulated_frame < frame_end:
                self.stroke_middle_contact_counts[stroke] = max(
                    self.stroke_middle_contact_counts[stroke],
                    int(middle_nut_contacts),
                )
        ...
        for side in ("LEFT", "RIGHT"):
            ...
            position_error, angle_error = self._transform_error(
                self._robot_hand_root(side, body_q[self.robot_j7_bodies[side]]),
                target_array,
            )
            self.maximum_robot_hand_position_error = max(
                self.maximum_robot_hand_position_error,
                position_error,
            )
        ...
        relative_rotation = np.asarray(
            tuple(_quat_multiply(wp.quat_inverse(bolt_rotation_host), nut_rotation_host)),
            dtype=np.float32,
        )
        twist_angle = _signed_twist_angle(
            relative_rotation,
            self._thread_initial_relative_rotation_host,
    ...
    def test_final(self) -> None:
        ...
        if not math.isclose(actual_prethread_advance, expected_prethread_advance, abs_tol=1.0e-7):
            raise ValueError(
                f"Unexpected initial thread advance: {actual_prethread_advance:.6f} m, "
                f"expected {expected_prethread_advance:.6f} m"
            )
        ...
        if not math.isclose(initial_flush_error, -INITIAL_BOLT_TIP_PROTRUSION, abs_tol=1.0e-7):
            raise ValueError(
                "Unexpected initial bolt-tip protrusion: "
                f"{-initial_flush_error:.6f} m, expected {INITIAL_BOLT_TIP_PROTRUSION:.6f} m"
            )
        ...
        if any(count < minimum_contact_frames for count in self.stroke_middle_contact_frame_counts):
            raise ValueError(
                "A middle-finger stroke did not maintain face contact: "
                f"{self.stroke_middle_contact_frame_counts} < {minimum_contact_frames}"
            )
        if self.maximum_other_right_hand_nut_contact_count != 0:
            raise ValueError(
                "A right-hand shape other than the middle finger contacted the nut: "
                f"{self.maximum_other_right_hand_nut_contact_count}"
            )
        ...
        if len(tightening_stroke_rotations) != RIGHT_STROKE_COUNT or any(
            ...
            raise ValueError(
                "A lateral friction stroke rotated the nut less than 60 degrees: "
                f"{[math.degrees(rotation) for rotation in tightening_stroke_rotations]}"
            )
        if -self.accumulated_nut_rotation < math.radians(RIGHT_STROKE_COUNT * MIN_STROKE_ROTATION_DEGREES):
            raise ValueError(
                "Full-angle friction strokes did not accumulate enough tightening: "
                f"{math.degrees(-self.accumulated_nut_rotation):.3f} tightening degrees"
            )
        if self.bolt_tip_protrusion <= 1.0e-4:
        ...
        right_position_error, right_angle_error = self._transform_error(
            self._robot_hand_root("RIGHT", body_q[self.robot_j7_bodies["RIGHT"]]),
            expected_right,
        )
        ...
        if self.maximum_bolt_displacement > MAX_BOLT_GRIP_TRANSLATION or self.maximum_bolt_rotation > math.radians(
            ...
            raise ValueError(
                "Left hand did not retain the bolt: "
                f"translation={self.maximum_bolt_displacement:.6f} m, "
                f"rotation={math.degrees(self.maximum_bolt_rotation):.3f} degrees"
            )
        collision_mask = int(newton.ShapeFlags.COLLIDE_SHAPES) | int(newton.ShapeFlags.COLLIDE_PARTICLES)
        ...
        if self.maximum_robot_hand_position_error > 0.002 or self.maximum_robot_hand_angle_error > math.radians(1.0):
            raise ValueError(
                "Full-W1 hands did not follow their IK targets: "
                f"translation={self.maximum_robot_hand_position_error:.6f} m, "
                f"rotation={math.degrees(self.maximum_robot_hand_angle_error):.3f} degrees"
            )
