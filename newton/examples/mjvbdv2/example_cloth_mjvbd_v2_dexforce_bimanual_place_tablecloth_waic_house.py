...
"""Place a tablecloth with Dexforce W1 and MJVBDV2 in the WAIC house.

The robot's fixed arm and free-base motion are solved once in an *IK-only*
model, then replayed as cached targets in the independent MJVBDV2 scene. The
...
Run, from the repository root::
    ...
    uv run --extra examples -m newton.examples \
        cloth_mjvbd_v2_dexforce_bimanual_place_tablecloth_waic_house
"""
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        if self.solver.features.backend != "mjvbd_kinematic_soft":
            raise RuntimeError(
                f"Tablecloth placement requires the mjvbd_kinematic_soft backend, got {self.solver.features.backend}"
            )
    ...
    def _configure_contact_materials(self):
        """Translate the reference VBD contact damping to MJVBDV2's current units."""
