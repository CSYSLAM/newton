...
This self-contained example includes its scene construction, realtime IK,
two-pass trajectory, contact schedule, and dynamic MJVBDV2 runner in this
file. It does not import or execute another example module.

Run, from the repository root::

    uv run --extra examples -m newton.examples \
        mjvbd_v2_tshirt_fold_dynamic
"""
...
class _KinematicExample:
    ...
    def _set_legacy_contact_damping(self):
        """Translate the authored scene's proportional damping into current VBD units."""
...
class Example(_KinematicExample):
    """Run the self-contained folding scene with dynamic robot joints."""
    ...
    def _configure_dynamic_contact_response(self):
        """Use the authored contact law for the dynamic proxies."""
        ...
        # The authored scene uses legacy proportional damping. Convert it
        # to the current VBD stiffness-scaled units after applying any override.
        damping = self.model.shape_material_kd.numpy()
        legacy_mixed_kd = 0.5 * (self.args.contact_kd + LEGACY_SHAPE_CONTACT_KD)
    ...
    @staticmethod
    def create_parser():
        parser = _KinematicExample.create_parser()
        parser.set_defaults(
            graph_capture=True,
        ...
        parser.add_argument(
            ...
            help="VBD iterations per substep; uses the authored scene default.",
        )
        ...
        parser.add_argument(
            ...
            action=argparse.BooleanOptionalAction,
            default=True,
            help="Enable cloth self-contact; enabled in the kinematic ",
        )
        parser.add_argument(
            "--self-contact-interval",
            type=int,
            default=-1,
            help="Self-contact detection interval; -1 matches the kinematic ",
        )
        parser.add_argument(
            "--contact-ke",
            type=float,
            default=3.0e5,
            help="Robot/cloth contact stiffness; matches the kinematic ",
        )
        parser.add_argument(
            "--contact-kd",
            type=float,
            default=LEGACY_SOFT_CONTACT_KD,
            help="Legacy proportional contact damping; matches the kinematic ",
        )
        parser.add_argument(
            "--max-tracking-error",
            type=float,
