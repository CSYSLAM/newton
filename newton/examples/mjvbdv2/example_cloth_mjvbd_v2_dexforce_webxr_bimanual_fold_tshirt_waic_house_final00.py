...
"""Teleoperate the WAIC-house Dexforce W1 T-shirt scene from Quest.

The full W1 visual meshes, physical table, and live two-sided cloth mesh are
rendered by Quest WebXR. Each grip clutches the corresponding W1 arm and each
trigger independently closes that hand. The 271 MiB visual-only house USD is
intentionally omitted from the headset payload to avoid exhausting WebGL
resources; it does not participate in the Newton physics scene.
...
WEBXR_CAMERA_HEIGHT_METERS = 0.35
WEBXR_CAMERA_PITCH_OFFSET_DEGREES = -8.0
HEAD_YAW_LIMITS = (-0.5 * np.pi, 0.5 * np.pi)
HEAD_PITCH_LIMITS = (-0.25 * np.pi, np.deg2rad(25.0))
HEAD_MAX_SPEED_RADIANS_S = np.deg2rad(50.0)
MAX_FINGER_SPEED_DEG_S = 180.0
MAX_FINGER_CONTACT_SPEED_DEG_S = 60.0
...
WORKSPACE_LOWER_OFFSET = np.array((-0.95, -0.95, -0.45), dtype=np.float32)
WORKSPACE_UPPER_OFFSET = np.array((0.95, 0.95, 0.80), dtype=np.float32)
...
def _limit_hand_target_step(
    ...
    """Limit one hand like the inflatable-bag example, slowing further on cloth contact."""
    finger = wp.tid()
...
def head_pose_to_neck_targets(head_pose: Pose | None) -> tuple[float, float]:
    """Map a relative WebXR head pose to the W1 yaw and pitch coordinates."""
...
class Example(shirt_scene.Example):
    """Drive both W1 hands against the physical T-shirt with Quest controllers."""

    reset_in_place = True

    def __init__(self, viewer, args):
        if args.graph_capture:
            raise ValueError("WebXR T-shirt teleoperation requires --no-graph-capture")
        ...
        base_orientation = self._quat_array(self.base_rot)
        ...
        self._initial_teleop_positions = {hand: position.copy() for hand, position in self._teleop_positions.items()}
        self._initial_teleop_orientations = {
            hand: orientation.copy() for hand, orientation in self._teleop_orientations.items()
        }
        self._teleop_grasps = dict.fromkeys(HANDS, 0.0)
        table_center = self._vec3_array(self._world_vec(shirt_scene.OLD_TABLE_POS))
        ...
        self._shirt_particle_start = 0
        self._shirt_particle_end = self.model.particle_count
        self._shirt_local_indices = np.asarray(self.model.tri_indices.numpy(), dtype=np.int64).reshape(-1, 3)
        if (
            self._shirt_local_indices.size == 0
            or int(self._shirt_local_indices.min()) < self._shirt_particle_start
            or int(self._shirt_local_indices.max()) >= self._shirt_particle_end
        ):
            raise RuntimeError("T-shirt triangle topology does not match the particle range")
        self._shirt_local_indices = (self._shirt_local_indices - self._shirt_particle_start).astype(np.uint32)
        self._static_boxes = [
            {
                "position": table_center.tolist(),
                "orientation": self._quat_array(self.base_rot).tolist(),
                "scale": [2.0 * float(value) for value in shirt_scene.TABLE_HALF_EXTENTS],
                "color": [float(value) for value in shirt_scene.TABLE_COLOR],
            }
        ]
        ...
        self.trajectory_recorder = JsonlTrajectoryRecorder(
            trajectory_path,
            {
                ...
                "headJointLabels": [self.model.joint_label[joint] for joint in neck_joints],
                "headCoordinateIndices": self._neck_q_indices_host.tolist(),
                "shirtParticleStart": self._shirt_particle_start,
                "shirtParticleCount": self._shirt_particle_end - self._shirt_particle_start,
                "shirtTriangleIndices": self._shirt_local_indices.reshape(-1).tolist(),
                "houseVisualIncludedInWebXR": False,
            },
            flush_every=int(args.record_flush_every),
        )
        geometry_payload = None
        if args.webxr_server:
            self._trace_startup("WebXR W1/T-shirt geometry packing started")
            geometry_payload = self._build_webxr_geometry()
            self._trace_startup("WebXR W1/T-shirt geometry packing complete")
        ...
        if args.webxr_server:
            ...
            print(
                f"Quest WebXR T-shirt server: http://{args.webxr_host}:{args.webxr_port}/\n"
                f"ADB USB route: adb reverse tcp:{args.webxr_port} tcp:{args.webxr_port}\n"
                f"W1/T-shirt geometry: {len(geometry_payload) / (1024 * 1024):.1f} MiB\n"
                f"Deforming shirt: {self._shirt_particle_end - self._shirt_particle_start} vertices\n"
                "The visual-only 271 MiB house USD is omitted from Quest WebGL\n"
    ...
    def _trajectory_path(path_value: Path | None) -> Path:
        ...
        return Path("recordings") / f"webxr_bimanual_fold_tshirt_{timestamp}.jsonl"

    def _build_independent_hand_control(self) -> None:
        hand_size = len(self.HAND_SUFFIXES)
        indices = np.asarray(self.hand_indices.numpy(), dtype=np.int32)
        open_q = np.asarray(self.hand_open.numpy(), dtype=np.float32)
        grasp_q = np.asarray(self.hand_grasp.numpy(), dtype=np.float32)
        if len(indices) != len(HANDS) * hand_size:
            raise RuntimeError("Unexpected W1 bimanual hand coordinate count")
        ...
        for hand_index, hand in enumerate(HANDS):
            ...
            self._hand_indices_by_side[hand] = wp.array(indices[start:end], dtype=wp.int32, device=self.device)
            self._hand_open_by_side[hand] = wp.array(open_q[start:end], dtype=wp.float32, device=self.device)
            self._hand_grasp_by_side[hand] = wp.array(grasp_q[start:end], dtype=wp.float32, device=self.device)
            self._desired_hand_q_by_side[hand] = wp.zeros(hand_size, dtype=wp.float32, device=self.device)

            hand_shapes = []
            for shape in self.robot_collision_shapes:
                body = int(shape_body[shape])
                label = self.model.body_label[body].lower() if body >= 0 else ""
                if hand in label and any(keyword in label for keyword in HAND_CONTACT_KEYWORDS):
                    hand_shapes.append(shape)
            if not hand_shapes:
                raise RuntimeError(f"Dexforce W1 did not provide collision shapes for the {hand} hand")
    ...
    def _robot_coordinate_indices(self) -> tuple[int, ...]:
        ...
        if not coordinates:
            raise RuntimeError("The T-shirt scene did not provide W1 coordinates")
    ...
    def _build_webxr_geometry(self) -> bytes:
        """Pack complete W1 visuals and the T-shirt's live topology."""
        ...
        shirt_positions = np.asarray(
            self.state_0.particle_q.numpy()[self._shirt_particle_start : self._shirt_particle_end],
            dtype=np.float32,
        )
        self._shirt_webxr_mesh_index = len(meshes)
        meshes.append(
            (
                shirt_positions,
                self._vertex_normals(shirt_positions, self._shirt_local_indices),
                self._shirt_local_indices,
            )
        )
        shapes.append(
            {
                "body": -1,
                "role": "shirt",
                "mesh": self._shirt_webxr_mesh_index,
                "position": [0.0, 0.0, 0.0],
                "orientation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "color": [float(value) for value in shirt_scene.SHIRT_COLOR],
                "doubleSided": True,
            }
        )
        roles = {str(shape["role"]) for shape in shapes}
        if not {"robot", "shirt"}.issubset(roles):
            raise RuntimeError(f"WebXR T-shirt scene is missing required mesh roles: {sorted(roles)}")
    ...
    def _viewer_camera_state(self) -> dict[str, list[float]]:
        ...
        if camera is not None and hasattr(camera, "get_front") and hasattr(camera, "get_up"):
            ...
            position = shirt_scene.CAMERA_POS
            pitch = np.deg2rad(shirt_scene.CAMERA_PITCH)
            yaw = np.deg2rad(shirt_scene.CAMERA_YAW)
            front = np.array(
    ...
    def _prepare_frame(self) -> None:
        """Retarget both newest Quest controller poses into the W1 TCPs."""
    ...
    def _update_runtime_ik_inputs(self):
        """Write live bimanual targets in place of the scripted fold trajectory."""
        self._prepare_frame()
        left = wp.transform(
        ...
        grip = max(float(value) for value in self._teleop_grasps.values())
        self._set_runtime_ik_target_poses(left, right)
        self._set_runtime_ik_grip(grip)
        return 0.0, grip

    def _solve_runtime_ik_frame(self):
        """Solve both arms, then advance each hand toward its independent grip target."""
        ...
        maximum_step = HEAD_MAX_SPEED_RADIANS_S * self.frame_dt
    ...
    def _consume_teleoperation_mode(self) -> None:
        ...
        if teleoperation_active:
            ...
            print(f"T-shirt teleoperation resumed in existing process (request {request_id})", flush=True)
            ...
            print(
                f"T-shirt controls disarmed while CUDA simulation remains active (request {request_id})",
                flush=True,
            )
        else:
            self.view_mode = OBSERVER_VIEW_MODE
            ...
            self.trajectory_recorder.pause()
            print(
                f"T-shirt teleoperation parked without destroying CUDA state (request {request_id})",
                flush=True,
            )
    ...
    def reset_physics(self, *, source: str) -> None:
        ...
        self._teleop_positions = {hand: position.copy() for hand, position in self._initial_teleop_positions.items()}
        self._teleop_orientations = {
            hand: orientation.copy() for hand, orientation in self._initial_teleop_orientations.items()
        }
        self._teleop_grasps = dict.fromkeys(HANDS, 0.0)
        self._neck_targets = self._neck_neutral.copy()
        self._desired_neck_targets = self._neck_neutral.copy()
        self._set_runtime_ik_target_poses(self.left_home, self.right_home)
        self._set_runtime_ik_grip(0.0)
        self._set_materials(*self._materials_for_script_time(0.0, 0.0))
        for retargeter in self.retargeters.values():
            retargeter.reset()
        ...
        print(f"T-shirt scene reset in place (episode {self.episode_index}, source={source})", flush=True)
    ...
    def step(self) -> None:
        ...
        if self._startup_sync_pending:
            self._trace_startup("first CUDA T-shirt simulation step started")
        super().step()
        if self._startup_sync_pending:
            self._trace_startup("first CUDA T-shirt step complete; device synchronization started")
        ...
        shirt_positions = particle_q[self._shirt_particle_start : self._shirt_particle_end]
        target_poses = [
            [
            ...
            self.trajectory_recorder.append(
                {
                    ...
                    "shirtParticleQd": particle_qd[self._shirt_particle_start : self._shirt_particle_end]
                    .reshape(-1)
                    .tolist(),
                }
            )
        if should_publish:
            self._publish_scene_state(body_q, shirt_positions, target_poses)

    def _publish_scene_state(
        self,
        body_q: np.ndarray | None = None,
        shirt_positions: np.ndarray | None = None,
        target_poses: list[list[float]] | None = None,
    ) -> None:
        if body_q is None:
            body_q = np.asarray(self.state_0.body_q.numpy(), dtype=np.float32)
        if shirt_positions is None:
            shirt_positions = np.asarray(
                self.state_0.particle_q.numpy()[self._shirt_particle_start : self._shirt_particle_end],
                dtype=np.float32,
            )
        ...
        self.webxr_server.publish_scene(
            {
                ...
                "sceneInfo": {
                    ...
                    "title": "双手叠 T 恤遥操作",
                    "description": "Quest 双眼显示完整 W1、物理桌面和实时双面 T 恤。也可切换机器人眼睛第一人称。",
                    "controls": [
                        ["左右 Grip", "按住并移动对应机器人手臂"],
                        ["左右 Trigger", "独立控制对应手指捏取 T 恤"],
                        ["左摇杆", "观察模式下转动视角"],
                        ["X / 视角按钮", "切换观察模式与机器人第一人称"],
                        ["A", "开始 / 暂停 / 继续轨迹录制"],
                        ["B", "用当前头部位姿重新对齐 Newton 相机"],
                        ["右摇杆按下", "原地复位 W1 和完整 T 恤"],
                    ],
                },
                "frame": self.frame_index,
                "episode": self.episode_index,
                ...
                "deformableMeshes": [
                    {
                        "mesh": self._shirt_webxr_mesh_index,
                        "positions": shirt_positions.reshape(-1).tolist(),
                    }
                ],
    ...
    def close(self) -> None:
        if self._resource_finalizer.alive:
            print("[shutdown] T-shirt WebXR and trajectory cleanup started", flush=True)
            self._resource_finalizer()
            print("[shutdown] T-shirt WebXR and trajectory cleanup complete", flush=True)
    ...
    @staticmethod
    def create_parser():
        parser = shirt_scene.Example.create_parser()
        ...
        parser.add_argument("--webxr-port", type=int, default=8769)
        parser.add_argument(
...
if __name__ == "__main__":
    ...
    print("[startup] Newton T-shirt viewer and simulation device initialization started", flush=True)
    viewer, args = newton.examples.init(parser)
    print("[startup] Newton T-shirt viewer and simulation device initialization complete", flush=True)
