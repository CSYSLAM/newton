...
Run from the repository root::
    ...
    uv run --extra examples -m newton.examples.mjvbdv2.support.example_vbd_mjvbd_v2_dexforce_recorded_soft_then_rigid_cube_into_bag --viewer gl
"""
...
from newton.examples.mjvbdv2.support import example_vbd_mjvbd_v2_dexforce_grasp_rigid_into_bag_rigid as rigid_reference
from newton.examples.mjvbdv2.support import example_vbd_mjvbd_v2_dexforce_recorded_soft_cube_into_bag as recorded_soft
...
# The recorded trajectory peaks at 434 soft and 364 rigid contacts.
...
class Example(recorded_soft.Example):
    ...
    def __init__(self, viewer, args):
        ...
        soft_cube_mask = np.all(
            (triangles >= self.soft_cube_particle_start) & (triangles < self.soft_cube_particle_end),
            axis=1,
        )
        self.bag_render_triangle_indices = wp.array(
            triangles[bag_mask].reshape(-1),
            dtype=int,
            device=self.device,
        )
        self.soft_cube_render_triangle_indices = wp.array(
            triangles[soft_cube_mask].reshape(-1),
            dtype=int,
            device=self.device,
        )
