...
Run from the repository root::
    ...
    uv run --extra examples -m newton.examples.mjvbdv2.support.example_vbd_mjvbd_v2_right_hand_soft_cube_recorder --viewer gl
"""
...
RIGHT_HAND_URDF = Path(__file__).resolve().parents[4] / "assets" / "W1_right_hand" / "DexforceW1_right_hand.urdf"
...
class Example:
    ...
    def create_parser():
        ...
        parser.add_argument(
            "--pose-output",
            default=str(
                Path(__file__).resolve().parents[4] / "assets" / "vbd_mjvbd_v2" / "vbd_w1_right_hand_pose.json"
            ),
        )
        parser.add_argument(
            "--trajectory-output",
            default=str(
                Path(__file__).resolve().parents[4] / "assets" / "vbd_mjvbd_v2" / "vbd_w1_right_hand_trajectory.json"
            ),
        )
        parser.add_argument(
            "--keyframe-output",
