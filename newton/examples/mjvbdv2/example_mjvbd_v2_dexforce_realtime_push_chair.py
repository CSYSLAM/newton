...
"""Push a dynamic WAIC chair with realtime Dexforce W1 IK and MJVBDV2.
...
articulated pose is injected as a kinematic MJVBDV2 proxy; the 8 kg chair
remains a free VBD rigid body and is moved only by hand contact.

Run, from the repository root::

    uv run --extra examples -m newton.examples \
        mjvbd_v2_dexforce_realtime_push_chair
"""
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        if self.solver.features.backend != "vbd_kinematic_full":
            raise RuntimeError(
                f"Realtime chair pushing requires vbd_kinematic_full, got {self.solver.features.backend}"
            )
