...
"""Upload fixed-topology material meshes sharing positions in one transfer."""
...
from .opengl import RendererGL, RenderVertex, _register_cuda_gl_buffer, fill_vertex_data


@wp.kernel(enable_backward=False)
def pack_shared_vertices(
    count: int,
...
class SharedMeshUpload:
    def __init__(self, viewer, names):
        self.viewer = viewer
        ...
        self.packed = None
        self.meshes = None
        self.shared_vbo = bool(viewer.device.is_cuda and viewer._enable_cuda_interop & viewer.CudaInterop.DYNAMIC_MESH)
        self.gather_normals = True
        ...
        if self.shared_vbo:
            if self.buffer is None:
                ...
                if self.registration is None:
                    raise RuntimeError("Shared mesh VBO requires CUDA/OpenGL interop")
    ...
    def log_mesh(self, name, points, indices, *args, **kwargs):
        ...
        if name != self.names[-1]:
            return None
        if len(self.pending) != len(self.names):
            raise ValueError("Shared mesh renderer requires all registered material parts")
        ...
        if any(a is not b for a, b in zip(meshes, self.meshes, strict=True)):
            raise ValueError("Shared mesh batch requires unchanged mesh objects")
        offset = 0
        for key, mesh in zip(self.names, meshes, strict=True):
            ...
            if positional or len(q) != mesh.num_points or len(faces) != mesh.num_indices or mesh.dynamic:
                raise ValueError("Shared mesh batch requires fixed topology and generated normals")
            if set(options) - {"color", "roughness", "metallic", "backface_culling"}:
                raise ValueError("Shared mesh batch does not accept texture, explicit normals or topology updates")
        ...
        if self.gather_normals:
            ...
            if any(
                ...
                raise ValueError("Shared mesh normal gather requires the same particle positions in every material")
            wp.launch(
                pack_shared_vertices,
