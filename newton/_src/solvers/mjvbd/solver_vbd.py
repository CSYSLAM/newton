...
class SolverVBD(SolverBase):
    ...
    **soft mode**. Joint hard/soft mode is initialized from the optional
    ...
    def _init_particle_system(
        ...
        particle_adjacency_host = self._compute_particle_force_element_adjacency()
        self.particle_adjacency = particle_adjacency_host.to(self.device)
        (
    ...
    def _compute_tile_material_check_fast_path_flags(self) -> tuple[int, int, int, int]:
        ...
        if self.model.tri_count > 0 and self.model.tri_materials is not None:
            ...
            tri_all_active = bool(np.all((tri_materials[:, 0] > 0.0) | (tri_materials[:, 1] > 0.0)))

        edge_all_active = True
        if self.model.edge_count > 0 and self.model.edge_bending_properties is not None:
            edge_bending_properties = self.model.edge_bending_properties.numpy()
            edge_all_active = bool(np.all(edge_bending_properties[:, 0] > 0.0))

        tet_all_active = True
        if self.model.tet_count > 0 and self.model.tet_materials is not None:
    ...
    def _body_particle_contact_launch_dim(self, soft_contact_max: int, soft_contact_count: int | None = None) -> int:
        """Return the processing width for body-particle contact kernels.

        When the active contact count is known on the host, the launch width is
        tightened to that count while staying warp-aligned. Otherwise, the
        solver falls back to the previous fixed-width heuristic.
        """
        if soft_contact_max <= 0:
            return 0

        if soft_contact_count is not None:
            active_count = min(soft_contact_max, max(0, soft_contact_count))
            if active_count == 0:
                return 0

            warp_aligned_count = ((active_count + 31) // 32) * 32
            target_launch_dim = min(2048, warp_aligned_count)
            return min(soft_contact_max, max(32, target_launch_dim))

        sm_count = max(1, getattr(self.device, "sm_count", 1))
        # Fallback when the active count is unavailable.
        target_launch_dim = max(256, min(2048, sm_count * 32))
        return min(soft_contact_max, target_launch_dim)

    def _sync_body_particle_contact_launch_state(self, contacts: Contacts | None) -> None:
        """Cache the active body-particle contact count and launch width for this step."""
        self.body_particle_contact_active_count_host = 0
        self.body_particle_contact_launch_dim = 0
    ...
    def _build_particle_elasticity_color_groups(
        self, particle_adjacency_host: ParticleForceElementAdjacencyInfo
    ) -> tuple[list[wp.array], list[wp.array]]:
        """Partition each particle color group by whether its particles touch tetrahedra."""

        def _empty_group() -> wp.array:
            return wp.empty(shape=(0,), dtype=wp.int32, device=self.device)
        ...
        tet_offsets = particle_adjacency_host.v_adj_tets_offsets.numpy()
        has_tet_adjacency = ((tet_offsets[1:] - tet_offsets[:-1]) >> 1) > 0

        surface_groups: list[wp.array] = []
        volumetric_groups: list[wp.array] = []
        for group in self.model.particle_color_groups:
            group_np = self._to_numpy(group, dtype=np.int32)
            group_has_tet = has_tet_adjacency[group_np]

            surface_np = group_np[~group_has_tet]
            volumetric_np = group_np[group_has_tet]

            surface_groups.append(
                wp.array(surface_np, dtype=wp.int32, device=self.device) if surface_np.size > 0 else _empty_group()
            )
            volumetric_groups.append(
                wp.array(volumetric_np, dtype=wp.int32, device=self.device)
                if volumetric_np.size > 0
                else _empty_group()
            )
    ...
    def step(
        ...
        self._sync_body_particle_contact_launch_state(contacts)
        ...
        if (not self.particle_enable_self_contact) or (not self._has_active_self_contact_host):
            ...
            wp.launch(
                ...
                inputs=[
                    ...
                    max_displacement,  # max_displacement: float (input threshold)
                ],
                outputs=[
    ...
    def _apply_particle_displacements_after_color(
        ...
        particles changed, so a subset update avoids rescanning all particles.
        """
        if self.particle_enable_self_contact and self._has_active_self_contact_host:
            self._penetration_free_truncation(particle_q_out)
            return
    ...
    def _initialize_particles(self, state_in: State, state_out: State, dt: float):
        ...
        if self.particle_enable_self_contact:
            self._collision_detection_penetration_free(state_in, reset_detection_anchor=False)
        else:
        ...
        wp.launch(
            ...
            inputs=[
                ...
                self.pos_prev_collision_detection if self.particle_enable_self_contact else None,
                state_in.particle_q,
    ...
    def _initialize_rigid_bodies(
        ...
        if model.particle_count > 0 and particle_refresh and contacts is not None:
            soft_contact_launch_dim = self.body_particle_contact_launch_dim
            soft_contact_active_count = self.body_particle_contact_active_count_host

            if soft_contact_launch_dim == 0 or soft_contact_active_count == 0:
                return
            ...
            if self.body_particle_contact_penalty_k.shape[0] < soft_contact_active_count:
                self._raise_if_capturing_resize(
                    "body-particle contact state",
                    self.body_particle_contact_penalty_k.shape[0],
                    soft_contact_active_count,
                )
                self._init_body_particle_contact_state(soft_contact_active_count)
            wp.launch(
    ...
    def _solve_particle_iteration(
        ...
        if contacts is not None and self.body_particle_contact_launch_dim > 0:
            soft_contact_launch_dim = self.body_particle_contact_launch_dim
            wp.launch(
            ...
            if self.use_particle_tile_solve:
                ...
                if surface_group.size > 0:
                    wp.launch(
                        ...
                        outputs=[
                            self.particle_displacements,
                            state_in.particle_q if use_local_particle_truncation else None,
                        ],
                        device=self.device,
                    )

                volumetric_group = self.volumetric_particle_color_groups[color]
                if volumetric_group.size > 0:
                    wp.launch(
    ...
    def _solve_rigid_body_iteration(
        ...
        if skip_rigid_solve:
            if model.particle_count > 0 and contacts is not None and self.body_particle_contact_launch_dim > 0:
                soft_contact_launch_dim = self.body_particle_contact_launch_dim
        ...
        if contacts is not None:
            ...
            if model.particle_count > 0 and self.body_particle_contact_launch_dim > 0:
                soft_contact_launch_dim = self.body_particle_contact_launch_dim
                wp.launch(
                    kernel=update_duals_body_particle_contacts,
                    dim=soft_contact_launch_dim,
    ...
    def _collision_detection_penetration_free(self, current_state: State, *, reset_detection_anchor: bool = True):
        # particle_displacements is based on pos_prev_collision_detection.
        # During particle initialization the subsequent forward_step overwrites
        # both arrays, so we can skip the extra full-array writes there.
        if reset_detection_anchor:
        ...
        if self.device.is_capturing:
            # During CUDA graph capture we cannot read active counts back to the
            # host, so keep the self-contact path enabled and let the kernels
            # early-out when there are no active VT/EE pairs.
            self._has_active_self_contact_host = True
        else:
            self._has_active_self_contact_host = bool(int(self.has_active_self_contact.numpy()[0]))
