...
def _get_pneumatic_counts(model: Model) -> tuple[int, int]:
    ...
    return (
        int(model.custom_frequency_counts.get("pneumatic:cavity", 0)),
        int(model.custom_frequency_counts.get("pneumatic:face", 0)),
    )
...
class SolverVBD(SolverBase, CouplingInterface):
    ...
    **soft mode**. Joint hard/soft mode is initialized from the optional
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            particle_enable_multilevel_correction: Enable one contact-aware two-level particle correction after the
        ...
        has_large_particle_color_group = self.device.is_cuda and any(
        ...
        self._pneumatic_incremental_volume_enabled = False
        ...
        if pneumatic_cavity_count > 0:
            self._init_pneumatic_system(
                model,
                pneumatic_cavity_count,
                pneumatic_face_count,
    ...
    def _init_particle_system(
        ...
        if multilevel_supported:
            ...
            if multilevel_mode == "auto":
                ...
                if self.particle_multilevel_auto_rejection_reason is None and _get_pneumatic_counts(model)[0] > 0:
                    self.particle_multilevel_auto_rejection_reason = "pneumatics_require_explicit_enable"
    ...
    def _init_pneumatic_system(
        ...
        cavity_count: int,
        face_count: int,
        deterministic: wp.DeterministicMode,
        enable_incremental_volume: bool,
    ) -> None:
        """Initialize optional closed-cavity pressure state."""
        from . import pneumatic_kernels  # noqa: PLC0415
        ...
        if namespace is None or any(not hasattr(namespace, field) for field in required_model_fields):
            raise ValueError(
                "Model contains pneumatic cavity rows but not the complete MJVBDV2 pneumatic schema. "
                "Use register_pneumatic_attributes() before authoring cavity rows."
            )

        self._set_module_options(
            {"deterministic": deterministic, "deterministic_max_records": 0},
            module=pneumatic_kernels,
        )
        self._pneumatic_kernels = pneumatic_kernels
        self._pneumatic_enabled = True
        self._pneumatic_cavity_count = cavity_count
        self._pneumatic_face_count = face_count
        ...
        face_cavities = np.asarray(namespace.face_cavity.numpy(), dtype=np.int32)
        face_triangles = np.asarray(namespace.face_triangle.numpy(), dtype=np.int32)
        if np.any((face_cavities < 0) | (face_cavities >= cavity_count)):
            raise ValueError("pneumatic:face_cavity contains an invalid cavity index.")
        if np.any((face_triangles < 0) | (face_triangles >= model.tri_count)):
            raise ValueError("pneumatic:face_triangle contains an invalid triangle index.")
        ...
        self._pneumatic_incremental_volume_enabled = (
            ...
            and model.device.is_cuda
            and not model.requires_grad
            and deterministic == wp.DeterministicMode.NOT_GUARANTEED
        )
        if self._pneumatic_incremental_volume_enabled:
            particle_colors = np.asarray(model.particle_colors.numpy(), dtype=np.int32)
            cavity_faces: list[list[int]] = [[] for _ in range(cavity_count)]
            color_cavity_faces: list[list[int]] = [[] for _ in range(len(model.particle_color_groups) * cavity_count)]
            for face, triangle in enumerate(face_triangles):
                cavity_faces[int(face_cavities[face])].append(face)
                vertices = triangle_indices[triangle]
                for color in np.unique(particle_colors[vertices]):
                    if 0 <= color < len(model.particle_color_groups):
                        row = int(color) * cavity_count + int(face_cavities[face])
                        color_cavity_faces[row].append(face)

            cavity_face_offsets = np.zeros(cavity_count + 1, dtype=np.int32)
            cavity_face_offsets[1:] = np.cumsum([len(faces) for faces in cavity_faces], dtype=np.int32)
            flat_cavity_faces = np.concatenate([np.asarray(faces, dtype=np.int32) for faces in cavity_faces])
            color_cavity_face_offsets = np.zeros(len(color_cavity_faces) + 1, dtype=np.int32)
            if color_cavity_faces:
                color_cavity_face_offsets[1:] = np.cumsum(
                    [len(faces) for faces in color_cavity_faces],
                    dtype=np.int32,
                )
            flat_color_cavity_faces = (
                np.concatenate([np.asarray(faces, dtype=np.int32) for faces in color_cavity_faces])
                if any(color_cavity_faces)
                else np.empty(0, dtype=np.int32)
            )
            self._pneumatic_cavity_face_offsets = wp.array(
                cavity_face_offsets,
                dtype=wp.int32,
                device=self.device,
            )
            self._pneumatic_cavity_faces = wp.array(flat_cavity_faces, dtype=wp.int32, device=self.device)
            self._pneumatic_color_cavity_face_offsets = wp.array(
                color_cavity_face_offsets,
                dtype=wp.int32,
                device=self.device,
            )
            self._pneumatic_color_cavity_faces = wp.array(
                flat_color_cavity_faces,
                dtype=wp.int32,
                device=self.device,
            )
    ...
    def _evaluate_pneumatic_cavities(self, particle_q: wp.array[wp.vec3], control: Control, dt: float) -> None:
        ...
        if self._pneumatic_incremental_volume_enabled:
            wp.launch(
                kernel=self._pneumatic_kernels.initialize_cavity_volume_and_pressure,
                dim=self._pneumatic_cavity_count * self._pneumatic_kernels._PNEUMATIC_CAVITY_INITIALIZE_BLOCK_DIM,
                block_dim=self._pneumatic_kernels._PNEUMATIC_CAVITY_INITIALIZE_BLOCK_DIM,
                inputs=[
                    self._pneumatic_cavity_count,
                    particle_q,
                    self.model.tri_indices,
                    pneumatic.face_triangle,
                    pneumatic.face_sign,
                    pneumatic.anchor_particle,
                    self._pneumatic_cavity_face_offsets,
                    self._pneumatic_cavity_faces,
                    dt,
                    pneumatic.mode,
                ...
                outputs=[
                    self._pneumatic_cavity_anchor_positions,
                    self._pneumatic_face_volume_contribution,
            ...
            wp.launch(
                kernel=self._pneumatic_kernels.accumulate_cavity_volume,
            ...
            self._evaluate_pneumatic_pressure(control_pneumatic, dt)

    def _evaluate_pneumatic_pressure(self, control_pneumatic, dt: float) -> None:
        """Update pressure observables from the current cavity volumes."""
        pneumatic = self.model.pneumatic
        wp.launch(
            kernel=self._pneumatic_kernels.evaluate_pressure,
    ...
    def _update_pneumatic_cavities_after_color(
        self,
        ...
        """Increment cached cavity volumes after one Gauss--Seidel color update."""
        control_pneumatic = getattr(control, "pneumatic", None)
        if control_pneumatic is None:
            raise ValueError("Control is missing pneumatic inputs; create it with model.control().")
        pneumatic = self.model.pneumatic
        wp.launch(
            kernel=self._pneumatic_kernels.update_cavity_volume_and_pressure_by_color,
            dim=self._pneumatic_cavity_count * self._pneumatic_kernels._PNEUMATIC_CAVITY_UPDATE_BLOCK_DIM,
            block_dim=self._pneumatic_kernels._PNEUMATIC_CAVITY_UPDATE_BLOCK_DIM,
            inputs=[
                color,
                self._pneumatic_cavity_count,
                particle_q,
                ...
                pneumatic.face_sign,
                self._pneumatic_cavity_anchor_positions,
                self._pneumatic_color_cavity_face_offsets,
                self._pneumatic_color_cavity_faces,
                dt,
                pneumatic.mode,
                pneumatic.rest_volume,
                pneumatic.reference_absolute_pressure,
    ...
    def _update_pneumatic_cavity_and_accumulate_forces(
        self,
        particle_q: wp.array[wp.vec3],
        ...
        """Fuse a single-cavity color update with the next pressure-force pass."""
        control_pneumatic = getattr(control, "pneumatic", None)
        if control_pneumatic is None:
            raise ValueError("Control is missing pneumatic inputs; create it with model.control().")
        pneumatic = self.model.pneumatic
        wp.launch(
            kernel=self._pneumatic_kernels.update_single_cavity_volume_pressure_and_accumulate_force,
            dim=self._pneumatic_kernels._PNEUMATIC_CAVITY_UPDATE_BLOCK_DIM,
            block_dim=self._pneumatic_kernels._PNEUMATIC_CAVITY_UPDATE_BLOCK_DIM,
            inputs=[
                update_color,
                particle_ids,
    ...
    def _accumulate_pneumatic_forces(
        self,
        particle_q: wp.array[wp.vec3],
        particle_ids: wp.array[wp.int32],
        particle_forces: wp.array[wp.vec3] | None = None,
        particle_hessians: wp.array[wp.mat33] | None = None,
    ) -> None:
        """Add cavity pressure terms to the current particle color."""
        pneumatic = self.model.pneumatic
        wp.launch(
            kernel=self._pneumatic_kernels.accumulate_pressure_force_and_hessian,
            ...
            inputs=[
                ...
                self._pneumatic_gauge_pressure,
                self._pneumatic_curvature,
                self.particle_forces if particle_forces is None else particle_forces,
                self.particle_hessians if particle_hessians is None else particle_hessians,
            ],
            device=self.device,
        )

    def _prepare_pneumatic_step(self, state_in: State) -> None:
        """Snapshot volume before VBD updates particle positions."""
    ...
    def _finalize_pneumatic_state(self, state_out: State, control: Control, dt: float) -> None:
    ...
    def _init_rigid_system(
        ...
        if model.particle_count > 0 and model.shape_count > 0:
            self._init_body_particle_contact_state(_count_world_compatible_particle_shape_pairs(model))
    ...
    def coupling_prepare_proxy_contacts(
        ...
        *,
        ...
        if (
            not self.one_way_proxy_bodies
            and contacts is not None
            and getattr(contacts, "_enable_rigid_soft_full_surface_contact", False)
        ):
    ...
    def _ensure_particle_contact_adjacency_capacity(self, soft_contact_max: int) -> None:
        ...
        if not self._particle_contact_gather_supported or self._particle_contact_next.shape[0] >= soft_contact_max:
            return
        self._raise_if_capturing_resize(
            "particle-contact adjacency",
            self._particle_contact_next.shape[0],
            soft_contact_max,
        )
        self._particle_contact_next = wp.empty(soft_contact_max, dtype=wp.int32, device=self.device)
        self._particle_contact_adjacency_initialized = False

    def _should_gather_particle_contacts(self, contacts: Contacts | None) -> bool:
        """Use gather only when the stream cannot contain edge or face contacts."""
        return (
            self._particle_contact_gather_supported
            and contacts is not None
            and contacts.soft_contact_max > 0
            and not getattr(contacts, "_enable_rigid_soft_full_surface_contact", False)
        )

    def _active_soft_contact_worker_dim(self, soft_contact_max: int) -> int:
        """Return a graph-stable worker count for the soft-contact dual update."""
        if soft_contact_max <= 0 or self.model.particle_count <= 0:
            return 0
        if not self._use_active_soft_contact_prefix:
    ...
    def _initialize_rigid_bodies(
        ...
        use_particle_contact_gather = self._should_gather_particle_contacts(contacts)
        if (
        ...
        if not particle_refresh and use_particle_contact_gather and not self._particle_contact_adjacency_initialized:
            particle_refresh = True
        ...
        if model.particle_count > 0 and particle_refresh and contacts is not None:
            ...
            if use_particle_contact_gather:
                self._ensure_particle_contact_adjacency_capacity(soft_contact_launch_dim)
                self._particle_contact_head.fill_(-1)
                adjacency_launch_dim = self._active_soft_contact_worker_dim(soft_contact_launch_dim)
            ...
            # Full-surface contacts retain the contact-major color mask path:
            # gathering would evaluate one edge/face record once per corner.
            if (
                not use_particle_contact_gather
                and self._use_particle_body_contact_color_masks
                and soft_contact_launch_dim > 0
            ):
                wp.launch(
    ...
    def _solve_particle_iteration(
        ...
        for color in range(len(self.model.particle_color_groups)):
            use_particle_contact_gather = particle_contact_gather_ready
            if self._pneumatic_enabled:
            ...
            if (
                ...
                and color + 1 < len(self.model.particle_color_groups)
            ):
    ...
    def _apply_particle_multilevel_correction(
        ...
        """Apply one contact-aware coarse correction at the final particle iterate."""
        correction = self.particle_multilevel
