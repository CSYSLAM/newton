...
class CoupledTranslationPCG:
    def __init__(self, solver, correction, ritz, fusion):
        ...
        inverse_mass = solver.body_inv_mass_effective.numpy()[dynamic]
        particle_mass = solver.model.particle_mass.numpy()
        if (
            not len(dynamic)
            or not np.all(np.isfinite(inverse_mass))
            or np.any(inverse_mass <= 0)
            or not np.all(np.isfinite(particle_mass))
            or np.any(particle_mass <= 0)
        ):
            raise ValueError("Coupled translation requires finite positive particle and solved free-body masses")
        ...
        self.pcg = (
            ComponentPCG(ritz, self.count, self.particle_count) if solver.enable_cuda_fast_path else TwoLevelPCG(ritz)
        )
