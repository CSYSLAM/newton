...
"""
D3Q27 lattice data and the per-world fluid struct for the LBM solver.

This is a port of the D3Q27 core from the Open HOME-LBM project
(``envs/lbm3d/lbm_core_3d.py``); the numerical state layout is kept
byte-for-byte equivalent. See :class:`newton.solvers.SolverLBM`.
"""
...
@wp.struct
class HomeFlow:
    """Store D3Q27 fluid and immersed-mesh state for one world.
    ...
    # cheap, warp-uniform (broadcast) cached loads.
    n_dirs: int
    ...
    def Initialize(self, nx: int, ny: int, nz: int, n_objects: int = 1) -> None:
        ...
        self.bc_value = wp.array((
            wp.vec3(0.0, 0.0, 0.0),  # left (x=0)
            wp.vec3(0.0, 0.0, 0.0),  # right (x=nx-1)
            wp.vec3(0.0, 0.0, 0.0),  # top (y=ny-1)
            wp.vec3(0.0, 0.0, 0.0),  # bottom (y=0)
            wp.vec3(0.0, 0.0, 0.0),  # front (z=nz-1)
            wp.vec3(0.0, 0.0, 0.0),  # back (z=0)
        ), dtype=wp.vec3)
