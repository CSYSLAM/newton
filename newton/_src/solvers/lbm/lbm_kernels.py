...
"""
Warp kernels for the D3Q27 Lattice Boltzmann solver.

Ported from the Open HOME-LBM project (``envs/lbm3d/lbm_func_3d.py``); the
numerical kernels are kept byte-for-byte equivalent.
"""
...
@wp.kernel
def InitBoundary3D(flows: wp.array(dtype=HomeFlow)):
...
@wp.kernel
def InitFlow3D(flows: wp.array(dtype=HomeFlow)):
...
@wp.kernel
def ResetSingleWorldFlow3D(flows: wp.array(dtype=HomeFlow), reset_mask: wp.array(dtype=wp.int32)):
...
@wp.kernel
def stream_and_collide_3d(flows: wp.array(dtype=HomeFlow)):
    ...
    if flow.flag[x, y, z] == ML_FLUID:
        ...
        # Moment accumulators (see the LBM project's perf notes). The 27 directions
        # are iterated with a RUNTIME loop over flow.n_dirs and the D3Q27 constants
        # are read from global memory (flow.lat_e / lat_w / lat_inv). This avoids
        # Warp's auto-unroll of `range(27)` — which over-pipelines into ~760 B
        # register spills and runs ~14x slower — and avoids register-materialising
        # the 27-element vector constants.
        rho_acc = float(0.0)
...
@wp.kernel
def apply_bc_3d(flows: wp.array(dtype=HomeFlow)):
...
def apply_bc_core_3d(
    flow: HomeFlow, outer: wp.int32, dr: wp.int32,
...
@wp.func
def f_eq_home_3d(flow: HomeFlow, x1: wp.int32, y1: wp.int32, z1: wp.int32):
    pop = wp.types.vector(length=27, dtype=wp.float32)
...
@wp.kernel
def Swap_Mom_3D(flows: wp.array(dtype=HomeFlow)):
...
def init_force_3d(flow: HomeFlow):
...
@wp.kernel
def init_force_3d_batch(flows: wp.array(dtype=HomeFlow)):
    """
...
@wp.kernel
def get_u_img_xy_3d(flow: HomeFlow, z_slice: int):
    """
...
@wp.kernel
def get_u_img_yz_3d(flow: HomeFlow, x_slice: int):
    """
...
@wp.kernel
def get_vorticity_xy_3d(flow: HomeFlow, z_slice: int):
    """
...
@wp.kernel
def get_vorticity_yz_3d(flow: HomeFlow, x_slice: int):
    """
...
@wp.kernel
def get_vorticity_xy_with_solid_3d(flow: HomeFlow, z_slice: int, thickness: float):
    """
...
@wp.kernel
def get_vorticity_yz_with_solid_3d(flow: HomeFlow, x_slice: int, thickness: float):
    """
...
@wp.kernel
def get_u_projection_topdown_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_u_projection_max_topdown_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_vorticity_projection_topdown_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_u_projection_side_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_u_projection_front_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_vorticity_projection_side_3d(flow: HomeFlow):
    """
...
@wp.kernel
def get_vorticity_projection_front_3d(flow: HomeFlow):
    """
