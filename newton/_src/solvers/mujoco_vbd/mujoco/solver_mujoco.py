...
from ..coupling_types import CouplingEndpointKind, CouplingInterface
from . import kernels
...
class SolverMuJoCo(SolverBase, CouplingInterface):
    ...
    def register_custom_attributes(cls, builder: ModelBuilder) -> None:
        ...
        def resolve_joint_dof_label(_: str, context: dict[str, Any]):
            ...
            - PhysicsRevoluteJoint / PhysicsPrismaticJoint: 1 DOF 鈫?[prim_path]
            - PhysicsFixedJoint: 0 DOFs 鈫?[] (empty list, no DOFs to name)
            - PhysicsSphericalJoint: 3 DOFs 鈫?[prim_path:rotX, prim_path:rotY, prim_path:rotZ]
            - PhysicsJoint (D6): N DOFs 鈫?one entry per free axis, determined from limit attributes
        ...
        for i in range(tendon_count):
            ...
            if ttype == 0:
                ...
                for w in range(wrap_start, wrap_start + wrap_num):
                    ...
                    # else: unknown wtype 鈥?already rejected during pre-validation
    ...
    def __init__(
        ...
        *,
        ...
        (from FIXED loop joints) are excluded 鈥?their ``eq_data`` is
        ...
        # _invalidate_contact_fast_path() 鈥?which is called from outside the
        # captured graph (e.g. notify_model_changed) 鈥?would then touch stale
        ...
        * :attr:`~newton.StateFlags.JOINT_QD` resets ``state.joint_qd``.
        ...
        Args:
            ...
            world_mask: Optional boolean mask of shape ``(world_count + 1,)``
                ...
                reset. The legacy shape ``(world_count,)`` is also accepted.
        ...
        if world_mask is not None and world_mask.shape[0] not in (world_count, world_count + 1):
            raise ValueError(
                f"world_mask has length {world_mask.shape[0]}, expected {world_count} or {world_count + 1}."
            )
    ...
    def _invalidate_contact_fast_path(self):
        ...
        etc.) may be stale 鈥?e.g. after :meth:`notify_model_changed` updates
    ...
    def _convert_contacts_to_mjwarp(self, model: Model, state_in: State, contacts: Contacts):
        ...
        # Zero nacon before the kernel 鈥?the full path uses atomic_add to count
    ...
    def _notify_model_changed(self, flags: ModelFlags | int) -> None:
        ...
        if self.use_mujoco_cpu:
            ...
            # Must be called last 鈥?mj_setConst/set_const_0 computes CONNECT anchor2
            ...
            if flags and self.enable_sleeping and not self._initial_model_sync:
        ...
        *,
        ...
        def add_geoms(newton_body_id: int):
            ...
            for shape in shapes:
                ...
                if stype == GeoType.HFIELD:
                    ...
                    # MuJoCo size: (size_x, size_y, size_z, size_base) 鈥?all must be positive
            ...
            if j_type == JointType.FREE:
                ...
                for i in range(lin_axis_count):
                    ...
                    if joint_params["limited"] and joint_has_raw_limit_solref(ai):
                        ...
                        # must survive the spec 鈫?``MjModel`` 鈫?save_to_mjcf
                        ...
                        # idempotent 鈥?re-imported FORCE_SPACE joints recover
                    ...
                    effort_limit = joint_effort_limit[ai]
                    joint_params["actfrclimited"] = effort_limit > 0.0
                    if effort_limit > 0.0:
                    ...
                    if mode != int(JointTargetMode.NONE):
                        ...
                        # Scalar slot (PRISMATIC/REVOLUTE/D6 axis) 鈥?no quaternion conversion.
                        if mode == JointTargetMode.POSITION:
                ...
                for i in range(lin_axis_count, lin_axis_count + ang_axis_count):
                    ...
                    if mode != int(JointTargetMode.NONE):
                        ...
                        template_dof = ai
                        template_target_q = tq_start + i
                        # Scalar slot (REVOLUTE/D6 angular axis) 鈥?no quaternion conversion.
                        if mode == JointTargetMode.POSITION:
                            actuator_args["gainprm"] = [kp, 0, 0, 0, 0, 0, 0, 0, 0, 0]
                            actuator_args["biasprm"] = [0, -kp, -kd, 0, 0, 0, 0, 0, 0, 0]
                            spec.add_actuator(target=axname, **joint_target_actuator_kwargs(actuator_args, ai, True))
        ...
        for j in joints_loop:
            ...
            if j_type == JointType.FIXED:
                # Fixed loop joint 鈫?weld constraint (constrains all 6 DOFs).
                ...
                # while the WELD relpose is measured in body2's local frame 鈥?                # these differ whenever the anchor is not at the body origin.
        ...
        # These map Newton template joint index 鈫?MuJoCo qpos/qvel start.
        ...
        with wp.ScopedDevice(model.device):
            ...
            if target_filename:
                ...
                # 鈥?subsequent edits to those Newton gains would no longer
                ...
                # round-trip for ``FORCE_SPACE`` joints 鈥?MJCF cannot
    ...
    def _update_joint_dof_properties(self):
        ...
        # set_const copies qpos0 鈫?d.qpos and runs FK to compute derived fields,
    ...
    def _update_solref_from_invweight0(self):
        ...
        # ``k_eff = 1/(蟿虏路味虏)``. Surface the misconfiguration once via a
