...
class SolverVBD(SolverBase, CouplingInterface):
    ...
    **soft mode**. Joint hard/soft mode is initialized from the optional
    ...
    def _compute_tile_material_check_fast_path_flags(self) -> tuple[int, int, int, int]:
        """Precompute uniform active/material conditions for private tiled kernels."""
        ...
        tet_active = self.model.tet_count == 0 or bool(
            np.all((self.model.tet_materials.numpy()[:, 0] > 0.0) | (self.model.tet_materials.numpy()[:, 1] > 0.0))
        )
        return (
            int(all_particles_active),
            int(tri_active and edge_active),
            int(all_particles_active),
            int(tri_active and edge_active and tet_active),
        )
    ...
    def _active_soft_contact_launch_dim(self, contacts: Contacts | None) -> int:
        ...
        if self.device.is_capturing:
            return contacts.soft_contact_max
        return min(contacts.soft_contact_max, max(0, int(contacts.soft_contact_count.numpy()[0])))
    ...
    def step(
        ...
        self._soft_contact_launch_dim = self._active_soft_contact_launch_dim(contacts)
    ...
    def _penetration_free_truncation(self, particle_q_out=None, selected_particles: wp.array | None = None):
        """
    ...
    def _initialize_rigid_bodies(
        ...
        if model.particle_count > 0 and particle_refresh and contacts is not None:
            soft_contact_launch_dim = self._soft_contact_launch_dim
            # Build body-particle contact lists (only when SolverVBD integrates bodies).
            if soft_contact_launch_dim > 0 and not self.integrate_with_external_rigid_solver and model.body_count > 0:
            ...
            if self.body_particle_contact_penalty_k.shape[0] < contacts.soft_contact_max:
                self._raise_if_capturing_resize(
                    "body-particle contact state",
                    self.body_particle_contact_penalty_k.shape[0],
                    contacts.soft_contact_max,
                )
                self._init_body_particle_contact_state(contacts.soft_contact_max)
            if use_particle_contact_gather:
                self._ensure_particle_contact_adjacency_capacity(contacts.soft_contact_max)
                self._particle_contact_head.fill_(-1)
                adjacency_launch_dim = self._active_soft_contact_worker_dim(self._soft_contact_launch_dim)
    ...
    def _solve_rigid_body_iteration(
        ...
        if skip_rigid_solve:
            if model.particle_count > 0 and contacts is not None and self._soft_contact_launch_dim > 0:
                body_q = state_out.body_q if self.integrate_with_external_rigid_solver else state_in.body_q
                if body_q is None:
                    body_q = self._empty_body_q

                soft_contact_launch_dim = min(
                    self._soft_contact_launch_dim,
                    self._active_soft_contact_worker_dim(contacts.soft_contact_max),
                )
                wp.launch(
        ...
        if contacts is not None:
            ...
            if model.particle_count > 0 and self._soft_contact_launch_dim > 0:
                soft_contact_launch_dim = min(
                    self._soft_contact_launch_dim,
                    self._active_soft_contact_worker_dim(contacts.soft_contact_max),
                )
