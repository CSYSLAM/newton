...
def reset_particle_state(
    ...
    """Copy model-default particle state into selected worlds."""
    particle = wp.tid()
    if not _reset_world_selected(particle_world[particle], world_mask, reset_all, world_count):
        return
    if particle_q:
...
def detect_any_active_self_contacts(
    ...
    """Reduce VT/EE detector counters to one device-side boolean."""
    index = wp.tid()
