...
from ..coupling_types import CouplingInterface
    ...
    **soft mode**. Joint hard/soft mode is initialized from the optional
    ...
    def reset(
        ...
        *state*; they do not restore a previously supplied state. A requested field
        ...
        ``PARTICLE_Q`` / ``PARTICLE_QD`` copy model-default particle state for
        the selected worlds. Particle contact and elasticity history is rebuilt
        or rebaselined by the next step.
        ...
        Args:
            ...
            flags: :class:`~newton.StateFlags` (or ``int``) selecting which body
                fields to copy from the model defaults. VBD honors body pose,
                body velocity, particle position, and particle velocity flags;
        ...
        if model.particle_count > 0:
            ...
            if flags_value & int(StateFlags.PARTICLE_Q) and state.particle_q is not None:
                if state.particle_q.device != self.device:
                    raise ValueError(
                        f"state.particle_q is on device {state.particle_q.device}, expected solver device {self.device}."
                    )
