...
"""Pneumatic-cavity authoring helpers for :class:`SolverVBD`."""
...
class PneumaticMode(IntEnum):
    """Pressure laws supported by :class:`SolverVBD` pneumatic cavities."""
...
class PneumaticConfig:
    ...
    The pressure law is evaluated by :class:`SolverVBD` during every particle
...
def _validate_oriented_surface(
    triangles: Sequence[tuple[int, int, int]], signs: Sequence[float]
) -> None:
    ...
    for edge, directions in edge_uses.items():
        if len(directions) != 2 or directions[0] == directions[1]:
            raise ValueError(
                "face_signs must orient every shared edge in opposite directions; "
                f"edge {edge} is invalid."
            )


def _surface_volume(
    positions: np.ndarray, triangles: Sequence[tuple[int, int, int]], signs: Sequence[float]
) -> float:
...
def add_pneumatic_cavity(
    ...
    *,
    config: PneumaticConfig = PneumaticConfig(),
