...
遥操纸袋绕世界 Z 轴转过 90°，袋口朝向零食；旋转后平移到桌面支撑范围内，复位恢复此摆放。
在此基础上向机器人左侧（世界 +Y）移动 20 cm，便于双手夹袋。
纸袋没有固定顶点，零食是动态刚体，夹取和装袋通过接触与摩擦实现。
物理模型、夹爪 SDF、接触设置和每帧推进均复用 `example_mjvbd_v2_w1_bag_packing.py`，
默认 6 个子步、12 次 VBD 迭代；`--substeps`、`--iterations` 对两个入口含义相同。
两个入口的拎手及其连接处均使用抗弯刚度 `0.3`、弯曲阻尼 `0.01`，为原值的 10 倍，
便于夹住拎手扶起袋子；袋身材料、拎手拉伸刚度和接触参数保持原值。
原资产的两个拎手改为朝袋口方向拱起；袋子直立时，拎手顶端高出袋沿约 30 mm。
CUDA 上默认复用自动场景的物理和 IK 图执行路径；使用 `--no-cuda-graph`
或 `--no-graph-capture` 可关闭。启动脚本也支持 `NEWTON_WEBXR_GRAPH_CAPTURE=0`。
...
这个停止脚本会真正退出仿真，扫描 NVIDIA GPU 进程与 `/proc`，只处理当前用户的
W1 装袋遥操进程，包括无 PID 文件或 systemd 记录的残留。发送 TERM 后等待 5 秒，
仍未退出则发送 KILL；确认清理成功后移除运行记录和该场景的 ADB 映射。
GPU 查询不可用时仍会扫描 `/proc`；清理失败返回非零退出码。其他场景的停止脚本沿用待机行为。
...
手势间距 15 mm 对应闭合，100 mm 对应张开；两侧独立计算。
左手滑块范围为 3.5–45 mm，用于夹袋；右手夹罐子时为 30–45 mm，夹纸盒时为 28–45 mm，
均沿用自动 demo 对应物体的夹取开度。这些数值是单个滑块位移，不是两指总间距。
右手靠近袋面或提手并闭合时，下限与左手一致，为 3.5 mm；靠近零食时使用对应零食的下限。
夹爪位于零食包围体内时优先保护零食，避免旁边的袋面触发过度闭合。
闭合期间保持已选对象的限制，张开后可重新选择；附近没有可夹物体时使用 30 mm 的保守下限。
手柄 Trigger 和手势捏合均受此限制。
选物体由夹爪位置、零食包围体和袋面距离自动判断，无需点选。保持闭合搬运零食到袋旁时，
不会改成夹袋开度；松开后才重新选择。
启动和复位时右手也保持 30 mm 开度。
左前方悬浮面板只在裸手模式显示；手柄模式隐藏面板并禁用其射线点击。
夹爪仍按运动学目标运动，开度限制不会阻止手臂从侧面强行推入物体。
追踪丢失、会话隐藏或手腕跳变时保持当前姿态与夹爪开度，恢复后重新建立手腕基准。
...
场景保留 `example_mjvbd_v2_nut_bolt.py` 的完整 W1、动态 M20 螺母/螺栓、预旋入状态和真实 SDF 螺纹接触。头显版本在机器人前方增加了真实刚体桌面，桌面顶面与螺母接触包络从第一帧接触，未抓稳时预旋入组件也不会直接掉到地面；详细螺纹 SDF 与桌面之间已过滤，避免和螺母凸包产生重复承托力。左右 Grip 分别移动对应手根；左右 Trigger 从张开手型缓慢闭合到原例子的任务手型。手指无接触时限制为 90°/s，发生刚体接触后降为 30°/s。原例子的接触设计也保持不变：左手负责夹持螺栓，右手用中指末端沿螺母侧面作切向扫动。
...
本节描述其他场景的待机停止行为。W1 装袋停止脚本已改为退出并清理残留进程，见上文。
