...
def prune_fused_resets(solver):
    """Omit resets overwritten by the free-body fused kernel, only in its phase."""
    targets = {
        array.ptr
        for array in (
            solver.body_forces,
    ...
    original_zero = wp.array.zero_
    original_iteration = solver._solve_rigid_body_iteration
    skipped = [0]

    def zero(array):
        if array.ptr in targets:
            skipped[0] += 1
            return array
        return original_zero(array)

    def iteration(*args, **kwargs):
        with mock.patch.object(wp.array, "zero_", zero):
            return original_iteration(*args, **kwargs)

    solver._solve_rigid_body_iteration = iteration
    return skipped
