...
- Add experimental `newton.solvers.SolverIPC` for `uipc`-backed tetrahedral soft-body simulation with rigid shapes treated as collision obstacles; install the optional `uipc` Python bindings and construct `SolverIPC(model)` to opt in.
